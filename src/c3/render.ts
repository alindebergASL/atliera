...
export type C3PageState = (
  ...
  | { readonly page: "draft"; readonly record: C3GenerationRecord; readonly correctionNote: string; readonly notice?: string }) & C3PendingState;
...
function prepare(context: FrozenC3AccountContext, request: C3MeetingFormState, error: string | undefined, hasDraft: boolean,
  ...
  const pending = revisionPending && correctionNote !== undefined && correctionNote.length > 0 ? `<p class="warning"><strong>Revision pending — previous draft preserved.</strong> Cancel stops generation; reopen the draft to deliberately discard the revision.</p><details class="recorded-note"><summary>Correction included in this revision</summary><p>This exact owner-entered note remains session-only. It is not approval or account truth. Cancel stops generation but does not discard the pending revision.</p><pre>${esc(correctionNote)}</pre></details>` : "";
...
function contextChecks(context: FrozenC3AccountContext): string {
  const gaps = context.context.materialGaps;
  const conflicts = context.context.declaredContradictions;
  return gaps.length + conflicts.length === 0 ? "" : `<section class="checks" aria-label="Account context gaps and conflicts"><h2>Still to establish</h2><ul>${conflicts.map((text) => `<li><strong>Conflicting context.</strong> ${esc(text)}</li>`).join("")}${gaps.map((text) => `<li><strong>Unknown.</strong> ${esc(text)}</li>`).join("")}</ul></section>`;
}
...
function draftPage(context: FrozenC3AccountContext, record: C3GenerationRecord, note: string, recordedCorrection: string | undefined,
  revisionPending: boolean, pendingRevisionToken?: string, notice?: string): string {
  ...
  const supported = (title: string, item: C3SupportedText) => `<section class="draft-section"><h2>${esc(title)}</h2>${body(item)}<p class="support"><span>${esc(supportLabel(item))}</span>${evidenceLinks(item.evidenceRefs, numberById, sourceByEvidence, title)}</p></section>`;
...
export function renderC3Page(context: FrozenC3AccountContext, state: C3PageState, csrf: string, options?: C3RenderOptions): string {
  ...
  if (state.page === "home") return shell(context.context.account.accountName, home(context, state.hasDraft ?? false, recorded,
    state.revisionPending ?? false), csrf, context, recorded, state.page, state.hasDraft ?? false);
