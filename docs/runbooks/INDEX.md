...
| `workshop-public-proposal-durable-graph-write-execution-status.md` | active | Current durable-write executor over the operator arming; it is the slice where graph_ingestion_performed and durable_writes_performed first flip from false to true, stamps mediation_gate_level: L0 only on completed outcomes (refusals carry no L0 marker; idempotent no-ops reference the prior write's L0 without re-claiming it), and provides single-transaction-or-noop semantics with idempotency keyed by the contract's idempotency_key_shape. |
...
| `m5a-curated-proposal-flow-contract-status.md` | active | Current M5a step 1 no-call typed contract for the curated-source proposal flow end-to-end (materialize → validate → ratify → durable write → render), grounded in the existing committed hand-curated-public materialization input that M3 step 3a's executor already consumes; ratifies Path 1 (recorded proposals only — no fresh provider call on the M5a flow path) with the ADR-0003-conflation rationale; expresses the "honestly labeled as curated" claim as a typed property of the success criterion (per-card data-curated-provenance attribute required, render label text pinned) rather than prose; closes the boundary marker forbids_fresh_provider_call_on_flow_path: true so Path 1 is structurally enforced; closes the M4-acquisition-class markers; preserves M3 trust-tier discipline (forbids verified per-record provenance in render); authorizes no provider call, no graph mutation, no durable write, no readiness claim, and no flow execution — gates the M5a step 2 approval packet. |
| `m5a-curated-proposal-flow-approval-packet-status.md` | active | Current M5a step 2 drafted-and-unarmed approval packet over the step-1 contract, with the eventual_authorization scope pinned to "durable-write-of-recorded-proposal-and-render" (the M5a-Path-1 sharpening — no fresh provider call, no system-side acquisition); positive trust-tier pins (required_row_trust_label "model-proposed-human-ratified-evidence-pending" and required_per_record_provenance_status "source_document_only") carried as REQUIRED fields alongside the negative prohibition; contract-reference integrity bound via references_contract_artifact_id / proposal_set_id / account_id triple; nineteen counterfeit-aimed hostile probes against the verifier prove the threat-shape adaptation (approval-state counterfeit, not input-smuggling); imports the snapshot helpers from step 1 (one consolidated M5a-layer site, recorded as H3 retro input); authorizes no provider call, no graph mutation, no durable write, no readiness claim, no arming (its own slice), no flow execution. |
| `m5a-curated-proposal-flow-operator-arming-status.md` | active | Current M5a step 3 operator-arming artifact over the step-2 drafted packet and step-1 contract; authorizes exactly one future curated proposal-flow execution while performing no flow execution, durable write, render, provider call, system acquisition, private read, production write, readiness claim, or mediation_gate_level stamp; verifies canonical arming_artifact_id on the build and verify paths, carries positive trust-tier pins and Path-1 markers, and uses arming-state counterfeit hostile probes as the done criterion. |
