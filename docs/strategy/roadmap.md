...
## Current C3 completion — 2026-09-10 renewal
...
The following markers and recommendation record that consumed checkpoint only. They are **not current C3 completion limits** and do not override the renewal above. C2 and M4 call budgets remain permanently closed; the C3 renewal does not reopen either.
...
These slices are ordered implementation milestones, not authority by themselves. New scope requires an explicit owner decision; routine steps within an already authorized milestone follow the standing development policy and current C3 renewal above. A prior mechanism milestone or deterministic fixture cannot skip a later gate.
...
- Completion of a routine implementation slice does not terminate the already authorized C3 completion mission. Respect its absolute deadline and cumulative cap, actual runtime effect gates and standing policy; new scope beyond that mission requires a separate owner decision. Historical milestone-specific return-to-none checkpoints remain historical rather than current C3 stop rules.
