...
function workshop(state: Extract<C3PageState, { page: "workshop" }>): string {
  ...
  const empty = !state.hasDraft && work?.available && !work.storageError && savedWorks.length === 0;
  const rows = savedWorks.map(item => {
    ...
    const opener = currentVersion && work?.saved ? `<a class="saved-title" href="/?draft=1"${openLabel}>${esc(title)}</a>` : `<button type="button" class="saved-title"${openLabel} data-reopen-work="${esc(item.documentId)}" data-replaces-unsaved="${Boolean(state.hasDraft && !work?.saved)}" data-work-version="${work?.workVersion ?? 0}">${esc(title)}</button>`;
