...
The active product path is the repository-native `m5b:prepare` / `m5b:apply` surface. It reuses the M5b FedEx source admission and candidate/review composition, requires explicit source/output locations plus exact source, owner-authorization, commit and tree bindings, and requires a separate external human-ratification artifact before one local durable graph write. Apply revalidates every binding, refuses replay, reads durable state back, and renders the final Workshop page from that read-back. Neither command acquires a source, calls a provider, selects authority, uses Hermes infrastructure, or deploys.

The host-local v2-r3 archive is `FROZEN_UNQUALIFIED_PROVENANCE_ONLY`; host forensic qualification, archive qualification, control-plane recovery and V4 reconciliation are closed for this milestone. They are not product acceptance requirements. M5b is **🔶 in progress, not shipped**. Real source execution, human ratification, and the resulting local durable write each remain unauthorized until the exact future decisions in `docs/runbooks/m5b-repository-native-product-completion.md` are supplied.
...
| **M5b — Does-its-job-once, system-acquired sources** (explicit admitted source → validated Graph candidate → human ratification → durable local graph write/read-back → Workshop rendered from durable state) | 🔶 in progress — repository-native product path implemented for review; real execution unauthorized | `m5b:prepare` and `m5b:apply` have no implicit paths, acquisition, provider, Hermes, deployment, or authority-selection behavior. Committed synthetic fixtures prove admission, candidate/review binding, ratification binding, one local write/read-back, final rendering, and replay/tamper refusal. Host-local v2-r3 is provenance only; no v2-r4. Real source execution and external human ratification require separate future owner decisions. |
