...
test("roadmap independently records M5a in progress at the merged Step-3 baseline", () => {
  ...
  assert.ok(
    ...
    "the full merged Step-3 baseline must be pinned without making a soon-stale current-main claim",
  );
...
test("operator ratification, not the earlier agent recommendation, makes Step 4 next", () => {
  const provenance = section(decision, "##", "Decision provenance");
  const currentSlice = section(roadmap, "###", "Current ratified implementation slice — M5a Step 4");
  ...
  assert.ok(
    ...
    "operator-ratified next slice must be Step 4",
  );
  assert.ok(
    currentSlice.includes("before H3 implementation unless a concrete safety blocker emerges"),
    "Step 4 must precede H3 absent a concrete blocker",
  );
});
...
test("H3 plan state and H1/H3 sequencing are reconciled without rewriting history", () => {
  ...
  assert.ok(
    hTrackNarrative.includes("H3 implementation is queued until after the M5a capstone and M5a retro"),
    "H3 implementation must be queued after the capstone and retro",
  );
  assert.ok(
    hTrackNarrative.includes("unless a concrete Step-4 safety blocker requires H3"),
    "the only pre-capstone H3 exception must be a concrete Step-4 safety blocker",
  );
...
test("Step 4 positive scope is exactly one arming, proposal, local write, read-back, and Workshop artifact", () => {
  const roadmapScope = section(roadmap, "###", "Current ratified implementation slice — M5a Step 4");
...
test("the decision authorizes implementation work but no execution effect", () => {
