...
const DOC = join(
  ...
  "docs",
  "runbooks",
  "runtime-model-only-controlled-corpus-v2-approval-packet.md",
);

test("v2 corrected-run approval packet authorizes exactly one bounded model-only run", () => {
  const doc = readFileSync(DOC, "utf8");

  for (const required of [
    /Status: pre-run docs-only approval packet/i,
    /does not execute a provider call/i,
    /exactly one future corrected controlled-corpus model-only validation run/i,
    /v2 output contract merged in PR #192/i,
    /PR #189 recorded one completed v1/i,
    /PR #190 recorded the no-spend usefulness assessment as `unsupported\/invented`/i,
    /PR #191 recorded the no-spend remediation plan/i,
    /PR #192 added the no-call v2 controlled-corpus output contract/i,
    /The PR #188 approval was consumed/i,
    /It is not reusable/i,
    /approval_ref: docs\/runbooks\/runtime-model-only-controlled-corpus-v2-approval-packet\.md/i,
    ...
    /canonical account_ref/i,
    /nonempty supporting_excerpt_ids/i,
    /known same-account excerpt ids/i,
    /rejection of display-name-only account labels/i,
    /rejection of account_objects without provenance/i,
    /must not broaden corpus, tools, transport, provider, model, budget, data source, retrieval, or product scope/i,
    /no tools/i,
    /no shell/i,
    /no file access/i,
    /no web search/i,
    ...
    /v2 provenance validation fails/i,
    ...
    /authorizes_v2_corrected_run: true/i,
