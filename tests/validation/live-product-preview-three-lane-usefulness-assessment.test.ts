...
test("applies the usefulness gate to the three-lane sanitized preview facts", () => {
  ...
  assert.equal(assessment.preview_ref, "live-product-preview-three-lane-20260529a");
  assert.equal(assessment.preview_usefulness_classification, "useful");
  assert.equal(assessment.ok, true);
  assert.equal(assessment.status, "pass");
  assert.deepEqual(assessment.reasons, []);
  assert.deepEqual(assessment.metrics.output_counts, { excerpts: 3, claims: 3, account_objects: 3 });
