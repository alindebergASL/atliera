...
- **M5a Step 4 now exists as a fixture-bound capstone.** It consumes and re-verifies the contract, drafted packet, and armed-state artifact; independently pins the exact committed proposal input by canonical SHA-256 because Steps 1–3 IDs do not contain that digest; enforces execution-time expiry and one-shot consumption; and performs one local durable write, read-back, and Workshop render. It is not a generic approval framework and does not broaden this Step-1 contract's authority.
