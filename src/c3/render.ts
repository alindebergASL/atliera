...
export interface WorkDisplayState { readonly available: boolean; readonly documentId: string; readonly version: number; readonly workVersion: number; readonly saved: boolean; readonly savedWorks: readonly { documentId: string; version: number; audience: string }[]; readonly storageError?: string; }
...
(() => {
  ...
  evidenceDialog?.addEventListener('close', () => {
    ...
    evidenceOrigin?.removeAttribute?.('data-inspected');
  ...
  document.addEventListener?.('click', (event) => {
    ...
    if (content && typeof evidenceDialog?.showModal === 'function') {
      ...
      evidenceOrigin = link;
      link.setAttribute?.('data-inspected', 'true');
...
${ACCOUNT_READING_CLIENT_SCRIPT}
  ...
  document.addEventListener?.('click', (event) => {
    ...
    if (entry?.getAttribute('data-refine-section') && reviewStatus) {
      evidenceOrigin = entry; openRevisionSheet(entry.getAttribute('data-refine-section')); return;
    }
...
function shell(title: string, body: string, csrf: string, context: FrozenC3AccountContext, recorded: boolean,
  ...
  const boundary = `<details class="recorded-mode"><summary>${synthetic ? 'Synthetic local preview' : recorded ? 'Recorded preview' : 'Content and storage details'}</summary><p>${synthetic ? 'Hand-authored fixtures · No AI recordings. Exact replay has no live fallback.' : recorded ? 'Recorded responses · No live generation. Exact request matching has no live fallback.' : isCuratedContext(context) ? 'Agent-curated proposed/template context. No human approval, owner disposition, policy admission or model recording. Owner and date unassigned.' : 'Proposed and unreviewed local content.'} Private document storage never changes account truth or approves content. ${esc(storage)}</p></details>`;
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="c3-csrf" content="${esc(csrf)}"><meta name="c3-account" content="${esc(context.context.account.accountId)}"><meta name="c3-document" content="${esc(work?.documentId ?? '')}">${recorded ? '<meta name="c3-recorded-replay" content="true">' : ""}<title>${esc(title)} · Atliera</title><style>${CSS}</style></head><body><a class="skip-link" href="#main">Skip to content</a><aside class="workspace-sidebar"><a class="brand" href="/">atliera</a><p class="sidebar-context">Account workspace</p>${navigation}</aside><div class="workspace-frame"><header class="workspace-header"><span class="account-identity">${esc(context.context.account.accountName)}</span>${destination === 'overview' ? '<a class="button" href="/?prepare=1">Prepare brief</a>' : destination === 'research' ? '<a class="quiet-link" href="/">Back to overview</a>' : page !== 'workshop' ? '<a class="quiet-link" href="/?view=workshop">Back to Workshop</a>' : ''}</header>${page === "prepare" || page === "workshop" ? `<p class="storage-notice">${esc(storage)}</p>` : ""}${body}${fallbackInspector}${boundary}</div><script>${SCRIPT}</script></body></html>`;
}
...
function home(frozen: FrozenC3AccountContext, hasDraft: boolean, recorded: boolean, revisionPending: boolean, topic?: ResearchTopic, selectedReading?: string): string {
  ...
  const row = (note: AccountReading) => `<article id="reading-${esc(note.id)}" class="account-reading" tabindex="-1"><h3>${esc(note.title)}</h3>${note.period ? `<p class="discovery-period">${esc(note.period)}</p>` : ""}${noteBody(note)}${note.question ? `<p class="context-note"><strong>Still to confirm</strong><br>${esc(note.question)}</p>` : ''}</article>`;
  ...
  const tile = (note: AccountReading) => `<a class="priority-tile" href="${esc(researchUrl(readingTopic(note), note.id))}"><h3>${esc(note.title)} <span aria-hidden="true">→</span></h3><p>${esc(note.text)}</p>${note.limit ? `<p class="meta">${esc(note.limit)}</p>` : ''}</a>`;
  const summarySection = (group: 'people' | 'technology', title: string, empty: string) => `<section id="account-${group}" class="summary-section"><h2>${title}</h2>${notes(group).slice(0, 2).map(note => `<a class="summary-row" href="${esc(researchUrl(group, note.id))}"><h3>${esc(note.title)} <span aria-hidden="true">→</span></h3><p>${esc(compactReading(note).text)}</p></a>`).join('') || `<p>${empty}</p>`}<a class="quiet-link" href="${esc(researchUrl(group))}">Explore ${group === 'people' ? 'people &amp; operating context' : 'technology &amp; services'} →</a></section>`;
  ...
  if (topic) {
    ...
    const topics = `<nav id="account-topics" class="research-topics" aria-label="Research topics">${(['initiatives', 'people', 'technology', 'sources'] as const).map(id => `<a href="${esc(researchUrl(id))}"${topic === id ? ' aria-current="page"' : ''}>${id[0]!.toUpperCase() + id.slice(1)}</a>`).join('')}</nav>`;
...
function workshop(state: Extract<C3PageState, { page: "workshop" }>): string {
  ...
  return `<main id="main" tabindex="-1"><h1>Workshop</h1><p class="lede">Prepare for a conversation and return to your account work.</p><div class="hero-actions"><a class="button" href="/?prepare=1">Prepare brief</a></div>${state.generation && !state.generation.available ? `<p class="context-note">${esc(state.generation.explanation)}</p>` : ''}${state.work?.available ? `<section class="workshop-list"><h2>Saved briefs</h2><p data-work-list-status role="status">${esc(state.work.storageError ?? '')}</p>${state.work.savedWorks.map(item => `<article class="workshop-item"><h3>Meeting brief</h3><p>${esc(item.audience)} · Version ${item.version}</p><button type="button" class="secondary" data-reopen-work="${esc(item.documentId)}" data-replaces-unsaved="${Boolean(state.hasDraft && !state.work?.saved)}" data-work-version="${state.work?.workVersion ?? 0}">Reopen saved brief</button></article>`).join('') || '<p>No saved briefs for this account.</p>'}</section>` : ''}<section class="workshop-list"><h2>Current work</h2>${state.hasDraft ? `<article class="workshop-item"><h3>Meeting brief${state.revisionPending ? ' · revision pending' : ''}</h3><p>The current session draft and its notes.</p><a class="quiet-link" href="/?draft=1">Reopen session draft →</a></article>` : '<p class="workshop-empty">No meeting brief yet. Start with the audience and outcome, or use an editable worksheet below.</p>'}${edited.map(brief => `<article class="workshop-item"><h3>${brief.kind === 'strategy' ? 'Strategy' : 'Next steps'} worksheet</h3><p>${esc(brief.audience || 'Audience not set')} · Session version ${brief.version}</p><a class="quiet-link" href="/?kind=${brief.kind}">Open worksheet →</a></article>`).join('')}</section><section class="account-section"><h2>Planning worksheets</h2><p class="meta">Editable templates · not AI-generated.</p><nav class="journey-nav" aria-label="Planning worksheets"><a href="/?kind=strategy">Strategy worksheet →</a><a href="/?kind=next-steps">Next-steps worksheet →</a></nav></section></main>`;
}
...
function prepare(context: FrozenC3AccountContext, request: C3MeetingFormState, error: string | undefined, hasDraft: boolean,
  recorded: boolean, recordedRequest?: C3MeetingFormState, correctionNote?: string, revisionPending = false, displayedRecordId: string | null = null, pendingRevisionToken: string | null = null, generation?: C3PageState["generation"]): string {
  ...
  const status = error ?? (!canGenerate ? unavailableMessage : recorded ? "Only the exact recorded request will replay. Edited inputs are refused; no live provider will be called." : generation?.explanation ?? "Generation route configured. Preparing creates a proposed session draft.");
...
function draftPage(context: FrozenC3AccountContext, record: C3GenerationRecord, note: string, recordedCorrection: string | undefined,
  revisionPending: boolean, pendingRevisionToken?: string, notice?: string, sectionNotes: SectionNotes = {}, state?: C3PageState): string {
  ...
  const secondarySetup = `<details data-generated-region="objective" class="technical-detail"${draft.objective.supportCategory === "unknown" ? " open" : ""}><summary>Proposed objective</summary>${body(draft.objective)}<p class="support"><span>${esc(supportLabel(draft.objective))}</span>${evidenceLinks(draft.objective.evidenceRefs, numberById, sourceByEvidence, "Meeting objective", draft.objective)}</p></details><div class="hero-actions"><a class="button secondary" href="/?prepare=1">${recordedCorrection === undefined ? "Edit meeting setup" : "Return to recorded request"}</a><a class="button secondary" href="#review">Add note</a></div>`;
  ...
  return `<main id="main" tabindex="-1"><div class="draft-head"><p class="eyebrow">Workshop / Brief</p><h1>Your meeting brief</h1>${workToolbar(state?.work)}<dl class="meeting-context" aria-label="Meeting setup from this draft’s request"><div><dt>Audience</dt><dd>${esc(request.audience)}</dd></div><div><dt>When · Duration</dt><dd>${esc(humanDate(request.meetingDate))} · ${String(request.durationMinutes)} minutes</dd></div><div class="outcome"><dt>Intended outcome</dt><dd>${esc(request.intendedOutcome)}</dd></div></dl></div>${notice ? `<p class="status-line" role="status">${esc(notice)}</p>` : ""}<div data-generated-region="state">${initialWarning}${revisionPending ? "<p>Revision pending. This is the preserved previous draft. Compare the proposal in Refine. Keep original discards the proposal and instruction; notes stay separate.</p>" : ""}<p class="meta">${esc(temporal)}</p></div>${pending}<div class="draft-grid"><div>${supported("Situation for this audience", draft.audienceThesis)}${secondarySetup}${supported("Opening", draft.opening)}<section class="draft-section" aria-labelledby="questions-heading"><div data-generated-region="questions"><h2 id="questions-heading">${draft.questions.length === 3 ? "Three" : String(draft.questions.length)} questions, in order</h2><ol class="questions">${draft.questions.map((question, index) => `<li id="question-${String(index + 1)}"><strong>${esc(question.question)}</strong>${learning(question.intendedLearning)}<p class="support"><span>Open question · Related evidence context</span>${evidenceLinks(question.evidenceRefs, numberById, sourceByEvidence, `Question ${String(index + 1)}`)}</p></li>`).join("")}</ol></div><button type="button" class="secondary" data-refine-section="Questions">Refine these questions</button>${sectionNoteEditor("Questions", record.recordId, sectionNotes, false)}</section>${supported("Useful close", draft.closeCriterion)}</div></div><section data-generated-region="checks" class="checks" aria-labelledby="checks-heading"><h2 id="checks-heading">Before relying on this brief</h2>${draft.risksUnknowns.map((item, index) => `${body(item)}<p class="support"><span>${esc(supportLabel(item))}</span>${evidenceLinks(item.evidenceRefs, numberById, sourceByEvidence, `Risk or unknown ${String(index + 1)}`, item)}</p>`).join("")}${draft.warnings.map((warning, index) => `<div class="warning">${esc(warning.message)}${warning.evidenceRefs.length === 0 ? "" : `<p class="support">${evidenceLinks(warning.evidenceRefs, numberById, sourceByEvidence, `Draft warning ${String(index + 1)}`)}</p>`}</div>`).join("")}</section>${contextChecks(context)}<details class="evidence-list" data-generated-region="evidence" aria-labelledby="evidence-heading"><summary id="evidence-heading">Evidence behind the brief</summary><p class="meta">Exact excerpts and source dates. References attached to an inference or question supply context, not direct proof.</p>${draft.selectedEvidenceRefs.map((id, index) => { const item = sourceByEvidence.get(id)!; return `<details id="evidence-${String(index+1)}"><summary>Evidence ${String(index+1)} · ${esc(item.source.title)}</summary><a class="evidence-return" data-evidence-return href="#questions-heading" hidden>Return to questions</a><div data-evidence-content>${admittedSourceSectionContext(item)}<blockquote>${esc(item.excerpt.exactExcerpt)}</blockquote>${sourceDetail(item.source)}${context.context.rendererAnnotations.filter((annotation) => annotation.sourceId === item.source.sourceId && (annotation.evidenceIds.length === 0 || annotation.evidenceIds.includes(id))).map((annotation) => `<p class="meta">${esc(annotation.text)}</p>`).join("")}${retained(item.source)}</div><p class="meta"><a href="#questions-heading">Back to questions</a></p></details>`; }).join("")}${draft.selectedEvidenceRefs.length === 0 ? '<p>No evidence selected by this draft. Do not treat the proposed content as established account fact.</p>' : ""}${otherSources.length === 0 ? "" : `<details><summary>Other retained account sources (${String(otherSources.length)})</summary>${otherSources.map((source) => `<h3>${esc(source.title)}</h3>${sourceDetail(source)}${retained(source)}`).join("")}</details>`}</details><dialog data-evidence-dialog aria-labelledby="evidence-panel-title"><div class="evidence-panel-head"><h2 id="evidence-panel-title">Evidence</h2><button type="button" data-evidence-close autofocus>Close panel</button></div><p data-evidence-support class="support"></p><div data-evidence-panel-body></div><div data-revision-panel hidden data-proposal-stale="${state?.proposalStale ?? false}" data-generation-available="${state?.generation?.available ?? recordedCorrection !== undefined}" data-proposal-id="${esc(state?.proposal?.recordId ?? '')}" data-proposal-instruction="${esc(state?.proposal?.revision?.correctionNote ?? '')}"><label for="revision-instruction">Your instruction</label><textarea id="revision-instruction" data-revision-instruction maxlength="1000">
${esc(state?.instruction ?? '')}</textarea><h3>Original opening</h3><p class="revision-original" data-revision-original>${esc(draft.opening.text)}</p>${recordedHelp ? `<div data-recorded-help>${recordedHelp}</div>` : ''}<div data-proposal-comparison${state?.proposal ? '' : ' hidden'}>${state?.proposal ? proposalComparison(record, state.proposal) : ''}</div><p data-revision-status role="status" aria-live="polite">The current brief stays unchanged until you apply.</p><div class="revision-actions"><button type="button" data-revise${revisionDisabled}>Propose revision</button><button type="button" class="secondary" data-stop-revision disabled>Stop revision</button><button type="button" data-apply-revision disabled>Apply revision</button><button type="button" class="secondary" data-discard-revision>Keep original</button></div></div></dialog><section class="review" id="review" aria-labelledby="review-heading"><h2 id="review-heading">Notes</h2><form data-note-form data-record-id="${esc(record.recordId)}" data-meeting-request="${esc(JSON.stringify(request))}" data-revised="${record.revision !== null}"${pendingRevisionToken ? ` data-pending-revision-token="${esc(pendingRevisionToken)}"` : ''}><label for="correction-note">Private annotation</label><textarea id="correction-note" data-correction-note name="note" maxlength="1000">
${esc(note)}</textarea><p class="status-line" data-review-status role="status" aria-live="polite"></p><button type="submit" class="secondary">Keep note</button></form></section><details data-generated-region="record" class="technical-detail"><summary>Record and timing details</summary><p>Temporal outcome: ${esc(draft.temporalOutcome.replace(/_/gu," "))}. Model prose and raw response are retained unchanged; display grouping does not establish source truth.</p><p>Record: ${esc(record.recordId)}. Context: ${esc(record.contextSha256)}.</p></details></main>`;
}
...
function renderPage(context: FrozenC3AccountContext, state: C3PageState, csrf: string, options?: C3RenderOptions): string {
  ...
  if (state.page === "prepare") return shell(`Prepare for ${context.context.account.accountName}`, prepare(context, state.request, state.error,
    state.hasDraft ?? false, recorded, options?.initialRequest, state.correctionNote, state.revisionPending, state.displayedRecordId, state.pendingRevisionToken, state.generation), csrf, context, recorded, state.page, state.hasDraft ?? false, options?.syntheticPreview, state.work);
  const draft = draftPage(context, state.record, state.correctionNote, options?.correctionNote, state.revisionPending ?? false,
    state.pendingRevisionToken, state.notice, state.sectionNotes, state);
