...
export function briefContext(frozen: FrozenC3AccountContext): string {
  const { proposal, admittedSources, materialGaps, declaredContradictions } = frozen.context;
  const evidence = admittedSources.flatMap(source => source.excerpts.map(excerpt => ({ source, excerpt })));
  const anchors = proposal.accountThesis.evidenceIds.flatMap(id => {
    const index = evidence.findIndex(item => item.excerpt.evidenceId === id);
    return index < 0 ? [] : [index];
  }).slice(0, 3);
  return `<details class="brief-context"><summary>Context for this brief</summary><div class="work-context"><p class="proposed-cue">Proposed account orientation · not reviewed</p><p>${esc(proposal.accountThesis.text)}</p><p class="support" data-thesis-state="${esc(proposal.accountThesis.state)}">${proposal.accountThesis.state === "source-backed fact" ? "Direct supporting evidence" : "Related evidence context"} · ${esc(proposal.accountThesis.state)}</p>${anchors.map(index => `<details class="brief-anchor"><summary>Evidence ${index + 1} · ${esc(evidence[index]!.source.title)}</summary><blockquote>${esc(evidence[index]!.excerpt.exactExcerpt)}</blockquote><a id="cite-brief-${index + 1}" data-evidence-link data-context="brief anchor ${index + 1}" href="#evidence-${index + 1}">Evidence ${index + 1} · ${esc(evidence[index]!.source.title)}</a></details>`).join("") || "<p>No direct thesis evidence is asserted.</p>"}${declaredContradictions.map(text => `<p class="warning"><strong>Conflicting context.</strong> ${esc(text)}</p>`).join("")}${materialGaps[0] ? `<p class="warning">${esc(businessGapLabel(materialGaps[0]))}</p>` : ""}<p class="meta">No new research. Source dates do not establish current status.</p><a href="#account-details">Full account context and gaps</a></div></details>`;
}

export function workshopKinds(current: string): string {
  return `<details class="secondary-worksheets"><summary>Other worksheets${current === "meeting" ? "" : ` · ${current === "strategy" ? "Strategy" : "Next steps"}`}</summary><nav class="journey-nav brief-kinds" aria-label="Workshop brief kind">${[["meeting", "Meeting", "/?prepare=1"], ["strategy", "Strategy", "/?kind=strategy"], ["next-steps", "Next steps", "/?kind=next-steps"]].map(([kind, label, url]) => `<a href="${url}"${current === kind ? ' aria-current="page"' : ""}>${label}</a>`).join("")}</nav></details>`;
}

/** Account context is read-only. Planning prose never feeds back into this projection. */
export function accountIntelSections(frozen: FrozenC3AccountContext, includeGaps = true, includeEvidence = true): string {
  ...
  return `<details id="account-details" class="account-details"><summary>Full account context and gaps</summary><section class="draft-section"><h2>Supplied account context</h2><p>${esc(admitted.sector ?? "Sector not established")} · ${esc(admitted.geography ?? "Geography not established")}</p>${admitted.notes.map((note) => `<p>${esc(note)}</p>`).join("")}<p class="meta">Context requested ${esc(context.account.requestedAt.slice(0, 10))}. Request date does not establish freshness. No new research has run.</p>${statements(context.proposal.establishedContext, "context")}</section><section class="draft-section"><h2>Developments and priorities to validate</h2><p class="meta">Proposed orientation, not approved account truth. No prior revision is available to establish change.</p>${statements(context.proposal.meaningfullyChanged, "development")}${statements(context.proposal.whyChangeMayMatter, "priority")}</section><section class="draft-section"><h2>Stakeholders and relationships</h2>${related.length ? `<ul>${related.map((item) => `<li><strong>${esc(item.name)}</strong> · ${esc(item.kind)}<br>${esc(context.relationships.find((relationship) => relationship.entityId === item.entityId)?.relationshipToAccount ?? item.relationshipToAccount)}</li>`).join("")}</ul><p class="meta">${isCuratedContext(frozen) ? "Proposed" : "Admitted"} entity relationships do not establish personal decision authority.</p>` : '<p>No stakeholder or related-entity relationships are established in this context. Confirm participants and decision authority.</p>'}</section>${includeGaps ? accountGaps(frozen) : ""}</details>${includeEvidence ? `<details class="evidence-list"><summary>Sources used</summary><h2>Account evidence</h2><p class="meta">Exact retained excerpts. Context for a proposal is not proof of its interpretation. Source dates do not establish current status.</p>${evidence.length === 0 ? '<p>No admitted sources. Use the open questions as a discovery plan; no evidence-backed conclusion is available.</p>' : evidence.map(({ source, excerpt }, index) => `<details id="evidence-${index + 1}"><summary>Evidence ${index + 1} · ${esc(source.title)}</summary>${source.untrustedInstructionsDetected ? '<p class="warning">Untrusted instructions detected. Inspection only; excluded from draft evidence.</p>' : ""}<a data-evidence-return hidden href="#main">Return to context</a><div data-evidence-content><blockquote>${esc(excerpt.exactExcerpt)}</blockquote><p class="meta">${esc(source.publisher)} · Published ${esc(source.publicationDate ?? "Undated")} · Event ${esc(source.eventDate ?? "Undated")} · Current through ${esc(source.evidenceCurrentThrough ?? "Not established")}</p>${context.rendererAnnotations.filter((item) => item.sourceId === source.sourceId).map((item) => `<p class="warning">${esc(item.text)}</p>`).join("")}<details><summary>Full retained source context</summary><p class="meta">Supplied bounded clean text; not original document completeness.</p><pre class="source-text" tabindex="0">${esc(source.fullBoundedCleanText)}</pre></details></div></details>`).join("")}</details>` : ""}`;
}

export function sectionNoteEditor(title: string, recordId: string, notes: SectionNotes, pending: boolean, storageAvailable = false): string {
  const text = notes[title] ?? "";
  const key = title.toLowerCase().replace(/[^a-z]+/gu, "-");
  return `<div class="section-note"><p data-saved-copy class="user-copy"${text ? "" : " hidden"}>${text ? `User note · ${esc(text)}` : ""}</p><details><summary>Add note<span class="sr-only"> for ${esc(title)}</span></summary><p class="meta">Your annotation stays separate from the brief. ${storageAvailable ? "Save the brief to retain it across restart." : "This note lasts for this session."}</p><form data-local-edit data-edit-key="meeting-${key}" data-endpoint="/api/section-note" data-record-id="${esc(recordId)}" data-section="${esc(title)}"><label for="note-${key}">Your note for ${esc(title)}</label><textarea id="note-${key}" name="text" maxlength="1000"${pending ? " disabled" : ""}>
${esc(text)}</textarea><p data-local-status role="status"></p><div class="hero-actions"><button type="submit"${pending ? " disabled" : ""}>Add note</button><button type="button" class="secondary" data-local-cancel${pending ? " disabled" : ""}>Cancel edit</button></div></form></details></div>`;
}

function proposedStepForm(context: FrozenC3AccountContext, brief: PlanningBrief, suggestion?: PlanningBrief["sections"][number]): string {
  const proposal = brief.proposedNextStep;
  const evidence = context.context.admittedSources.flatMap((source) => source.excerpts.map((excerpt) => ({ source, excerpt })));
  const fields = [
    ["concern", "Concern", 2000, true], ["action", "Proposed action", 4000, true],
    ["owner", "Proposed owner (optional)", 160, false], ["targetDate", "Proposed target date (YYYY-MM-DD, optional)", 10, false],
    ["questionOrBlocker", "Question or blocker (optional)", 2000, false],
  ] as const;
  const label = (item: typeof evidence[number]) => `Evidence ${evidence.indexOf(item) + 1} · ${item.excerpt.exactExcerpt.length > 72 ? item.excerpt.exactExcerpt.slice(0, 72) + "…" : item.excerpt.exactExcerpt}`;
  return `<section class="draft-section proposal-section" aria-label="Proposed next step"><p class="meta">Session-only proposal · not an assignment or approval. No scheduling, outreach, or durable save.</p><details open class="proposal-editor"><summary>Author or edit proposed next step</summary><form data-local-edit data-proposed-next-step data-edit-key="next-steps-proposal" data-endpoint="/api/planning/next-steps" data-version="${brief.version}">${fields.map(([name, title, max, required]) => `${name === "owner" ? '<div class="proposal-owner-date">' : ""}<div class="field"><label for="proposal-${name}">${title}</label>${name === "targetDate" ? `<input type="text" id="proposal-${name}" name="${name}" maxlength="${max}" value="${esc(proposal?.[name] ?? "")}"${name === "targetDate" ? ' placeholder="YYYY-MM-DD" pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}"' : ""}>` : `<textarea id="proposal-${name}" name="${name}" maxlength="${max}"${name === "owner" ? ' rows="1"' : ""}${required ? " required" : ""}>
${esc(proposal?.[name] ?? "")}</textarea>`}</div>${name === "targetDate" ? "</div>" : ""}`).join("")}<div class="field"><label for="proposal-evidence">Supporting evidence (optional, up to 32)</label><p id="proposal-evidence-help" class="meta">Select retained excerpts that help explain this proposal. Use Control or Command to select multiple entries on desktop.</p><select id="proposal-evidence" name="evidenceIds" multiple size="${Math.max(2, Math.min(5, evidence.length))}" aria-describedby="proposal-evidence-help">${evidence.map((item) => `<option value="${esc(item.excerpt.evidenceId)}"${proposal?.evidenceIds.includes(item.excerpt.evidenceId) ? " selected" : ""}>${esc(label(item))}</option>`).join("")}</select>${evidence.length ? `<details><summary>Inspect supporting excerpts</summary><ul>${evidence.map((item, index) => `<li><a id="cite-proposal-${index + 1}" data-evidence-link data-context="Proposed next step" href="#evidence-${index + 1}">Inspect ${esc(label(item))}</a></li>`).join("")}</ul></details>` : '<p>No retained evidence available. Evidence is not set.</p>'}</div><p data-local-status role="status"></p><div class="hero-actions"><button type="submit">Keep proposed next step</button><button type="button" class="secondary" data-local-cancel>Cancel edit</button></div></form></details><p data-proposal-status>${proposal ? "User-authored proposed next step · session-only" : "No proposed next step kept."}</p><details class="kept-proposal"><summary>Kept proposal details</summary><dl>${fields.map(([name, title]) => `<dt>${title}</dt><dd class="user-copy" data-proposed-value="${name}">${esc(proposal?.[name]?.trim() ? proposal[name] : name === "owner" || name === "targetDate" ? "Unassigned" : "Not set")}</dd>`).join("")}<dt>Supporting evidence</dt><dd data-proposed-value="evidenceIds">${proposal?.evidenceIds.length ? proposal.evidenceIds.map((id) => esc(label(evidence.find((item) => item.excerpt.evidenceId === id)!))).join(" · ") : "Not set"}</dd></dl></details>${suggestion ? `<details class="strategy-suggestion"><summary>Strategy suggestion</summary><aside aria-label="Strategy suggestion"><p class="meta">${suggestion.authorship === "user" ? "User" : "Template"}-authored suggestion · not an accepted decision. Read-only here; author your proposal in the editor above. This suggestion is not saved as a proposed next step.</p><p class="user-copy">${esc(suggestion.text || "Not set")}</p></aside></details>` : ""}<p><a href="/">Return to Account Intel</a> · <a href="/?kind=strategy">Return to Strategy</a></p></section>`;
}

export function planningPage(context: FrozenC3AccountContext, brief: PlanningBrief, strategySuggestion?: PlanningBrief["sections"][number]): string {
  ...
  const title = brief.kind === "strategy" ? "Strategy brief" : "Next steps";
...
${esc(value)}</textarea></div>`;
  return `<main id="main" tabindex="-1"><p class="eyebrow">Workshop</p><h1>${title}</h1>${brief.kind === "strategy" ? '<p class="lede">Frame a direction, compare options, and decide what to validate.</p>' : ""}${workshopKinds(brief.kind)}${brief.kind === "strategy" ? '<p class="proposed-cue">Editable planning template · not AI-generated</p><p class="meta">User/template-authored, session-only. Use the proposed account context to shape your plan; edits never change Account Intel.</p>' : '<p class="proposed-cue">Your editable plan · not AI-generated. Account Intel stays unchanged.</p>'}${brief.kind === "next-steps" ? proposedStepForm(context, brief, strategySuggestion) : ""}<section class="draft-section" data-planning-version="${brief.version}"><h2>Audience and intended outcome</h2><p data-setup-summary>${brief.audience ? `${esc(brief.audience)} · ${esc(brief.intendedOutcome)}` : "Set who this is for and the result you want."}</p><details><summary>Edit brief setup</summary><form data-local-edit data-edit-key="${brief.kind}-setup" data-endpoint="/api/planning/${brief.kind}" data-version="${brief.version}">${field("audience", "Audience", brief.audience, 160, true)}${field("intendedOutcome", "Intended outcome", brief.intendedOutcome, 500, true)}${field("detail", brief.kind === "strategy" ? "Planning horizon or constraints (optional)" : "Timing or dependencies (optional)", brief.detail, 500)}<p data-local-status role="status"></p><div class="hero-actions"><button type="submit">Keep brief setup</button><button type="button" class="secondary" data-local-cancel>Cancel edit</button></div></form></details><p class="meta" data-detail-summary>${esc(brief.detail)}</p></section><div class="planning-work">${planningContext(context, brief)}<div class="planning-sections">${brief.kind === "next-steps" ? "<details><summary>Additional planning</summary>" : ""}${brief.sections.map((section) => `<section class="draft-section"><h2>${esc(section.title)}</h2>${brief.kind === "strategy" && section.id === "decision" ? '<p><a class="button" href="/?kind=next-steps&amp;from=strategy">Shape a proposed next step</a></p>' : ""}<p data-saved-copy class="user-copy">${esc(section.text || "Section cleared. No conclusion asserted.")}</p><p data-authorship class="meta">${section.authorship === "template" ? "Template-authored planning prompt" : "User-authored session edit"}</p><details><summary>Edit ${esc(section.title)}</summary><form data-local-edit data-edit-key="${brief.kind}-${section.id}" data-endpoint="/api/planning/${brief.kind}" data-version="${brief.version}" data-section="${section.id}">${field("text", `Your ${section.title.toLowerCase()}`, section.text, 4000).replaceAll("plan-text", `plan-${section.id}`)}<p data-local-status role="status"></p><div class="hero-actions"><button type="submit">Keep section edit</button><button type="button" class="secondary" data-local-cancel>Cancel edit</button></div></form></details></section>`).join("")}${brief.kind === "next-steps" ? "</details>" : ""}</div></div>${accountIntelSections(context)}</main>`;
}
