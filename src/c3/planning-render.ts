...
function proposedStepForm(context: FrozenC3AccountContext, brief: PlanningBrief, suggestion?: PlanningBrief["sections"][number]): string {
  ...
  const label = (item: typeof evidence[number]) => `${item.source.title} · ${item.excerpt.exactExcerpt.slice(0, 140)}`;
  return `${suggestion ? `<aside class="draft-section" aria-label="Strategy suggestion"><h2>Strategy suggestion</h2><p class="meta">${suggestion.authorship === "user" ? "User" : "Template"}-authored suggestion · not an accepted decision. Read-only here; author your proposal below. This suggestion is not saved as a proposed next step.</p><p class="user-copy">${esc(suggestion.text || "Not set")}</p></aside>` : ""}<section class="draft-section"><h2>Proposed next step</h2><p class="meta">Session-only proposal. Keeping it is not an assignment or approval; no scheduling, outreach, or durable save occurs.</p><p data-proposal-status>${proposal ? "User-authored proposed next step · session-only" : "No proposed next step kept."}</p><dl>${fields.map(([name, title]) => `<dt>${title}</dt><dd class="user-copy" data-proposed-value="${name}">${esc(proposal?.[name]?.trim() ? proposal[name] : name === "owner" || name === "targetDate" ? "Unassigned" : "Not set")}</dd>`).join("")}<dt>Supporting evidence</dt><dd data-proposed-value="evidenceIds">${proposal?.evidenceIds.length ? proposal.evidenceIds.map((id) => esc(label(evidence.find((item) => item.excerpt.evidenceId === id)!))).join(" · ") : "Not set"}</dd></dl><details open><summary>Author or edit proposed next step</summary><form data-local-edit data-proposed-next-step data-edit-key="next-steps-proposal" data-endpoint="/api/planning/next-steps" data-version="${brief.version}">${fields.map(([name, title, max, required]) => `<div class="field"><label for="proposal-${name}">${title}</label><textarea id="proposal-${name}" name="${name}" maxlength="${max}"${required ? " required" : ""}>
${esc(proposal?.[name] ?? "")}</textarea></div>`).join("")}<div class="field"><label for="proposal-evidence">Supporting evidence (optional, up to 32)</label><p id="proposal-evidence-help" class="meta">Select retained excerpts that help explain this proposal. Use Control or Command to select multiple entries on desktop.</p><select id="proposal-evidence" name="evidenceIds" multiple size="${Math.max(2, Math.min(5, evidence.length))}" aria-describedby="proposal-evidence-help">${evidence.map((item) => `<option value="${esc(item.excerpt.evidenceId)}"${proposal?.evidenceIds.includes(item.excerpt.evidenceId) ? " selected" : ""}>${esc(label(item))}</option>`).join("")}</select>${evidence.length ? `<details><summary>Inspect supporting excerpts</summary><ul>${evidence.map((item, index) => `<li><a id="cite-proposal-${index + 1}" data-evidence-link data-context="Proposed next step" href="#evidence-${index + 1}">Inspect ${esc(label(item))}</a></li>`).join("")}</ul></details>` : '<p>No retained evidence available. Evidence is not set.</p>'}</div><p data-local-status role="status"></p><div class="hero-actions"><button type="submit">Keep proposed next step</button><button type="button" class="secondary" data-local-cancel>Cancel edit</button></div></form></details><p><a href="/">Return to Account Intel</a> · <a href="/?kind=strategy">Return to Strategy</a></p></section>`;
}
