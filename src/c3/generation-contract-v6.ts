...
import type { C3DraftQuestion, C3DraftWarning, C3MeetingDraftCandidate, C3MeetingRequest, C3ModelRequest, C3ProposedDraft, C3RevisionContext, C3SupportedText, C3SupportCategory } from "./draft.ts";

export const v6Hash = (value: string): string => createHash("sha256").update(value, "utf8").digest("hex");
/** JSON.parse validates syntax; this scan additionally refuses duplicate object members without rewriting bytes. */
function parseV6Json(raw: string): unknown {
...
export function validateV6Integrity(rawText: string, context: FrozenC3AccountContext, meetingDate?: string): C3ProposedDraft {
  ...
  let parsed: unknown;
  try { parsed = parseV6Json(rawText); } catch { throw new Error("model response must be one strict JSON object"); }
  const root = object(parsed, "candidate");
  exactKeys(root, ["temporalOutcome", "objective", "audienceThesis", "opening", "questions", "risksUnknowns", "closeCriterion", "selectedEvidenceRefs", "assertions"], "candidate");
  ...
  validateGeneratorAssertions(root.assertions, displayFields(candidate));
...
function assertion(value: unknown, fields: readonly C3DisplayField[], finding = false): C3Assertion {
  const root = object(value, 'assertion');
  exactKeys(root, ['path', 'start', 'end', 'text', 'kind', 'evidenceRefs', 'entityScope', 'dateScope', 'modality',
    ...(finding ? ['verdict', 'reason'] : [])], 'assertion');
  const field = fields.find(field => field.path === root.path);
  if (!field || !Number.isInteger(root.start) || !Number.isInteger(root.end) ||
      (root.start as number) < 0 || (root.end as number) <= (root.start as number) || (root.end as number) > field.text.length ||
      root.text !== field.text.slice(root.start as number, root.end as number)) throw new Error('Assertion exact text binding mismatch');
  ...
  return root as unknown as C3Assertion;
}

function validateGeneratorAssertions(value: unknown, fields: readonly C3DisplayField[]): void {
  for (const item of array(value, 'candidate.assertions', 0, 128)) assertion(item, fields);
}
...
const candidateSchema = {
  ...
  assertions: [{ path: 'opening.text (or another display field path)', start: 0, end: 42, text: 'exact substring',
    kind: ASSERTION_KINDS.join(' | '), evidenceRefs: ['field evidenceId'], entityScope: 'exact entity and organizational boundary',
    dateScope: 'event/publication/currentness distinction or not applicable', modality: 'reported/planned/operational/conditional/proposed/unknown' }],
};

export function createV6ModelRequest(context: FrozenC3AccountContext, meetingRequest: C3MeetingRequest,
  revision: C3RevisionContext | null): C3ModelRequest {
  assertC3GenerationContext(context);
  const revisionSha256 = revision === null ? null : v6Hash(canonicalJson(revision));
  ...
  const prompt = [
    'GENERATION CONTRACT 6. Return one complete meeting brief as strict JSON, no markdown, extra fields, retries or repaired output.',
    ...
    'Use natural concise seller-facing language. Lead with a useful situation and spoken opening. A fifteen-minute meeting has three ordered main questions: current outcome, owner/constraint, and whether a next step helps. Include one or two explicitly optional conditional probes in opening or intendedLearning when useful. Longer meetings have three to seven questions. Close permits no follow-up.',
    'Separate proposed actions from their factual rationale in distinct sentences and assertion entries. A suggestion alone does not establish priority, ownership, buying intent or availability. Inspect assumptions hidden in questions, recommendations, learning purposes and relative clauses. Preserve uncertainty locally rather than borrowing a hedge from a different clause.',
    ...
    'All prose fields contain 3–1200 characters; 1–8 risksUnknowns. selectedEvidenceRefs exactly equals the union cited in fields. Cite only eligible IDs with no duplicates. Assertions identify every factual statement, inference, unknown, question presupposition and action rationale; separate action and rationale. Offsets are JavaScript UTF-16 code units, end-exclusive, bound to exact displayed text. An independent check will inspect the full brief and find claims missing from assertions.',
  ...
  return deepFreezeOwnData({ kind: 'atliera.c3.meeting-draft-model-request', schemaVersion: '2', generationContractVersion: '6',
    contextSha256: context.sha256, meetingRequestSha256: v6Hash(canonicalJson(meetingRequest)), meetingRequest,
    revision, revisionSha256, prompt });
}

export type C3VerificationSchemaVersion = '1' | '2';

export interface C3VerificationRequest {
  readonly kind: 'atliera.c3.evidence-verification-request';
  readonly schemaVersion: C3VerificationSchemaVersion;
  readonly generationContractVersion: '6';
  readonly contextSha256: string;
  readonly modelRequestSha256: string;
  readonly rawResponseSha256: string;
  readonly prompt: string;
}
export interface C3Verification {
  readonly request: C3VerificationRequest;
...
/** Frozen schema 1 wire contract: preserve prompt bytes for historical replay. */
function createC3VerificationRequestSchema1(modelRequest: C3ModelRequest, rawResponse: string,
  context: FrozenC3AccountContext): C3VerificationRequest {
  if (modelRequest.generationContractVersion !== '6' || modelRequest.contextSha256 !== context.sha256 ||
      canonicalJson(createV6ModelRequest(context, modelRequest.meetingRequest, modelRequest.revision)) !== canonicalJson(modelRequest)) {
    throw new Error('Verification generator request identity mismatch');
  }
  const candidate = validateV6Integrity(rawResponse, context, modelRequest.meetingRequest.meetingDate);
  const identity = { contextSha256: context.sha256, modelRequestSha256: v6Hash(canonicalJson(modelRequest)), rawResponseSha256: v6Hash(rawResponse) };
  const schema = { kind: 'atliera.c3.evidence-verification', schemaVersion: '1', ...identity,
    findings: [{ path: 'opening.text', start: 0, end: 42, text: 'exact candidate span',
  ...
  const prompt = [
    'Independently check the complete candidate against the supplied evidence. This is quality control, not approval or proof. Do not rewrite it, generate alternatives, fetch data or assume generator assertions are complete.',
    ...
    'Read EVERY DISPLAY FIELD from start to end, including temporalOutcome, questions and intendedLearning. Identify assertions missing from the generator list. Split mixed supported/unsupported clauses into separate findings. Check implicit factual premises in questions/actions, entities and unit-versus-account scope, publication-versus-event dates, planned-versus-operational modality, attributed relations, commercial availability, sparse evidence and contradictions. A hedge or attribution is not entailment.',
    ...
    'Produce findings that partition each DISPLAY FIELD completely, in ascending start offset, with no gaps or overlap. Include punctuation and whitespace. Offsets are JavaScript UTF-16 code units, end-exclusive. Bind each finding text exactly to its field substring. No field may be omitted. One mixed clause cannot borrow another clause’s support. non_assertion is only connective/punctuation or genuinely nonfactual text, never a way to omit an assertion or hidden assumption.',
  ...
  return deepFreezeOwnData({ kind: 'atliera.c3.evidence-verification-request', schemaVersion: '1', generationContractVersion: '6', ...identity, prompt });
}

function createC3VerificationRequestSchema2(modelRequest: C3ModelRequest, rawResponse: string,
  context: FrozenC3AccountContext): C3VerificationRequest {
  if (modelRequest.generationContractVersion !== '6' || modelRequest.contextSha256 !== context.sha256 ||
      canonicalJson(createV6ModelRequest(context, modelRequest.meetingRequest, modelRequest.revision)) !== canonicalJson(modelRequest)) {
    throw new Error('Verification generator request identity mismatch');
  ...
  return deepFreezeOwnData({ kind: 'atliera.c3.evidence-verification-request', schemaVersion: '2', generationContractVersion: '6', ...identity, prompt });
}

/** Fresh checks use schema 2; replay must explicitly select the retained request version. */
export function createC3VerificationRequest(modelRequest: C3ModelRequest, rawResponse: string,
  context: FrozenC3AccountContext, schemaVersion: C3VerificationSchemaVersion = '2'): C3VerificationRequest {
  switch (schemaVersion) {
    case '1': return createC3VerificationRequestSchema1(modelRequest, rawResponse, context);
    case '2': return createC3VerificationRequestSchema2(modelRequest, rawResponse, context);
    default: throw new Error('Unknown evidence verification schema version.');
  }
}


export function retainC3Verification(request: C3VerificationRequest, rawResponse: string | null): C3Verification {
  return deepFreezeOwnData({ request, requestSha256: v6Hash(canonicalJson(request)), rawResponse,
    rawResponseSha256: rawResponse === null ? null : v6Hash(rawResponse), failure: rawResponse === null ? 'unavailable' : null });
}

/** Frozen schema 1 validation, including offset binding and refusal ordering. */
function assertV6VerificationSchema1(modelRequest: C3ModelRequest, rawResponse: string, context: FrozenC3AccountContext,
  verification: C3Verification | undefined): void {
  ...
  const expected = createC3VerificationRequestSchema1(modelRequest, rawResponse, context);
  if (canonicalJson(expected) !== canonicalJson(verification.request) || verification.requestSha256 !== v6Hash(canonicalJson(expected))) {
  ...
  if (verification.rawResponse === null) {
    ...
    throw new Error('Independent evidence check unavailable; original candidate retained.');
  }
  if (verification.failure !== null || typeof verification.rawResponse !== 'string' ||
      verification.rawResponseSha256 !== v6Hash(verification.rawResponse)) throw new Error('Independent evidence check response identity mismatch.');
  ...
  let parsed: unknown;
  try { parsed = parseV6Json(verification.rawResponse); } catch { throw new Error('Independent evidence check must be strict JSON.'); }
  ...
  if (root.kind !== 'atliera.c3.evidence-verification' || root.schemaVersion !== '1' ||
  ...
  const fields = displayFields(validateV6Integrity(rawResponse, context, modelRequest.meetingRequest.meetingDate));
  ...
  for (const value of array(root.findings, 'evidence check findings', fields.length, 256)) {
    const item = assertion(value, fields, true);
    if (positions.get(item.path) !== item.start) throw new Error('Independent evidence check coverage has gaps or overlap.');
    positions.set(item.path, item.end);
  ...
  if (unsupported) throw new Error('Evidence check found contradicted or insufficiently supported content; original candidate retained.');
}

/** Schema 2 binds original segment text directly; cursors are validator-owned UTF-16 positions. */
function partitionAssertion(value: unknown, fields: readonly C3DisplayField[], positions: Map<string, number>): Omit<C3Assertion, 'start' | 'end'> {
  const root = object(value, 'assertion');
  ...
  return root as unknown as Omit<C3Assertion, 'start' | 'end'>;
}

function assertV6VerificationSchema2(modelRequest: C3ModelRequest, rawResponse: string, context: FrozenC3AccountContext,
  verification: C3Verification | undefined): void {
  if (!verification) throw new Error('Independent evidence check is required for this candidate.');
  exactKeys(object(verification, 'verification'), ['request', 'requestSha256', 'rawResponse', 'rawResponseSha256', 'failure'], 'verification');
  const expected = createC3VerificationRequestSchema2(modelRequest, rawResponse, context);
  if (canonicalJson(expected) !== canonicalJson(verification.request) || verification.requestSha256 !== v6Hash(canonicalJson(expected))) {
    throw new Error('Independent evidence check request identity mismatch.');
  }
  if (verification.rawResponse === null) {
...
/** Stored request schema selects replay semantics; never retry or fall back to another version. */
export function assertV6Verification(modelRequest: C3ModelRequest, rawResponse: string, context: FrozenC3AccountContext,
  verification: C3Verification | undefined): void {
  if (!verification) throw new Error('Independent evidence check is required for this candidate.');
  const schemaVersion = object(verification.request, 'verification request').schemaVersion;
  switch (schemaVersion) {
    case '1': return assertV6VerificationSchema1(modelRequest, rawResponse, context, verification);
    case '2': return assertV6VerificationSchema2(modelRequest, rawResponse, context, verification);
    default: throw new Error('Unknown evidence verification schema version.');
  }
}
