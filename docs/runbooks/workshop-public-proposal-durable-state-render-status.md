...
- `composeDurableStateView` descriptor-snapshots every input at the compose boundary before reading any field: the root `readerResult` object itself, the rows array, each row, the row's bundle, the bundle's account_objects array, and each account_object. This refuses a getter installed on the root `readerResult.rows` / `.checked_at` / `.source_path` / `.refusals`; a Proxy-backed root readerResult; an accessor-backed index on the rows array or on `account_objects[]`; and an accessor-backed inner field on the row or the account_object — all BEFORE any getter fires. `isProxy` + `Array.isArray` is not sufficient: a plain non-Proxy array can still carry getter-backed indices, and a plain non-Proxy object can carry getter-backed top-level fields. The render layer reads from the snapshot, not the input.
...
- a forged reader result with a getter installed on the TOP-LEVEL `readerResult.rows` (or a Proxy-backed `readerResult`) is refused at the root-object snapshot; the top-level getter never fires
