...
| **M4 — Evidence acquisition v1** (deterministic system-side fetcher as the first orchestrator-held MCP integration; allowlisted, sandboxed, provenance-tagged; adversarial injection corpus in definition of done; L0+L1 only; **its own capstone, its own visible artifact and retro — not co-shipped with any M5 milestone**) | ⬜ not started; implementation unauthorized | Future thin capability is exactly `public_http_fetch_v1`. H2 may draft its registry entry, descriptor and budget defaults, but must not implement or execute it. After H2 merges and receives independent review, a fresh operator/roadmap decision is required for M4 implementation. Any later live acquisition remains separately gated by one compact packet and explicit operator GO. ADR 0003 A1; direction memo §7. |
...
1. **Fork vs. build** for the M4 fetch server — deferred to the fresh M4 implementation decision after H2 merges and receives independent review. H2 may draft M4 registry/descriptor/budget defaults but may not choose, implement or execute the fetcher.
