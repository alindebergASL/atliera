...
`src/validation/controlled-corpus-remediation-plan.ts` now provides the deterministic no-spend controlled corpus weakness remediation helper for the next step before another live run, comparison, or expansion. `planControlledCorpusWeaknessRemediation(...)` consumes the already-produced weakness diagnosis and maps it to remediation areas `prompt_contract`, `proposal_schema`, `evidence_policy`, `rubric_thresholds`, `fixture_coverage`, and `substrate_contract`. It allows only no-execution action labels: `no_spend_prompt_contract_revision`, `proposal_schema_revision`, `rubric_clarification`, `evidence_policy_clarification`, `deterministic_fixture_update`, and `fix_hard_substrate_or_contract_blocker`. It blocks `live_provider_rerun`, `provider_comparison`, `corpus_expansion`, `launch_readiness_claim`, and `product_readiness_claim`, and preserves `approves_live_provider_call: false`, `approves_provider_spend: false`, `approves_expansion_or_comparison: false`, and `launch_readiness_claim: false`. The remediation plan does not authorize provider calls, does not authorize provider spend, does not approve comparison or expansion, does not approve a rerun, and does not imply launch readiness, product readiness, production readiness, broad model quality, or multi-account corpus readiness.

`src/agent/controlled-corpus-graph-propose-contract.ts` now provides the no-spend prompt/proposal remediation contract for the weak-but-valid result before another 2b-expanded rerun is considered. `buildControlledCorpusGraphProposePromptContract(...)` defines a role-specific `graph.propose` contract that asks for strict JSON with `excerpts`, `claims`, `account_objects`, and `lens_summaries`, grounding account objects in accepted account-object evidence through excerpt-backed claims and Signals, Maps, or Plays lens support. This contract is intended to repair prompt/proposal underproduction without spending: it does not authorize provider calls, does not authorize provider spend, does not approve a rerun, and does not approve comparison or expansion.
