...
The opaque auth context also keys a module-private, process-local attempt state. A verified accepted context begins ready and is synchronously marked spent before its first preflight. A terminal preflight failure or any transaction result other than `committed` or an exact `already_committed` durable replay leaves it spent, so reuse is refused before another preflight even after repair. Success restoration requires the transaction's canonical PR #303 receipt to match the rederived decision-bound intent, candidate, replay key, review handoff, graph identity, predecessor, and receipt digest; it deliberately does not require storage-current read-back to equal that historical decision. A successful result records the already-guarded disposable file's process-local device/inode/birthtime identity. That context may repeatedly perform exact durable replay while the same file still exists—including truthful historical/overtaken read-back after a later revision—but it cannot bootstrap against an absent or recreated file. A new verifier-issued context is required after failure or replacement. This state is neither an approval store nor a replay namespace and is not serialized into artifacts, HTML, or SQLite.
