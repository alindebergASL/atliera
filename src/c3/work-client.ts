...
export const WORKING_DOCUMENT_CLIENT_SCRIPT = `
  ...
  const markWorkDirty = () => { workEpoch++; workDirty = true; workNavigationApproved = false; if(workStatus) workStatus.textContent = workControls?.getAttribute('data-store-available') === 'true' ? 'Unsaved changes' : 'Session only'; };
  document.addEventListener?.('input', (event) => { if(reviewForm && event.target?.closest?.('main, [data-revision-panel]')) markWorkDirty(); });
  const currentRecord = () => reviewForm?.getAttribute('data-record-id');
  const controls = () => {
    const stale = !proposalId || proposalStale || instruction?.value !== proposalInstruction;
    ...
    const revise = document.querySelector('[data-revise]'); if(revise) { revise.disabled = reviewBusy || !instruction?.value.trim() || revisionPanel?.getAttribute('data-generation-available') !== 'true' || (recordedReplay && reviewForm?.getAttribute('data-revised') === 'true'); revise.textContent = pendingRevisionToken ? 'Regenerate proposal' : 'Propose revision'; }
    ...
    if (proposalId && stale && revisionStatus && !reviewBusy) revisionStatus.textContent = 'Instruction changed. Regenerate before applying; the current brief is unchanged.';
  ...
  const openRevisionSheet = (section) => {
    ...
    if(!evidenceDialog.open) { priorBodyOverflow = document.body?.style?.overflow || ''; evidenceScroll = [window.scrollX || 0,window.scrollY || 0]; evidenceDialog.showModal(); if(document.body?.style) document.body.style.overflow='hidden'; }
    instruction?.focus(); controls();
  };
  ...
  const displayProposal = (payload) => {
    ...
    proposalId=payload.proposalId; proposalStale=payload.stale === true || instruction.value!==payload.instruction; proposalInstruction=payload.instruction;
  ...
  document.querySelector('[data-revise]')?.addEventListener('click',async()=>{
    ...
    try {
      ...
      instructionQueue=instructionQueue.then(async()=>{
        ...
        syncedInstruction=submitted;pendingRevisionToken=payload.pendingRevisionToken;proposalId=null;proposalStale=instruction.value!==submitted;invalidateFormCache();markWorkDirty();return true;
      });
      const staged=await instructionQueue;
      if(staged && !stopRequested)await generateRevision();
    }catch(error){revisionStatus.textContent=error.message+' Current brief and typing kept.';}
  ...
  document.querySelector('[data-apply-revision]')?.addEventListener('click',async()=>{
    if(reviewBusy || !proposalId || proposalStale || instruction.value!==proposalInstruction || !canStartRevision())return;
    ...
    try{
      ...
      pendingRevisionToken=null;proposalId=null;syncedInstruction='';
    ...
    }catch(error){revisionStatus.textContent=error.message+' Local typing kept.';}
    finally{reviewBusy=false;controls();}
  });
  ...
  const saveWork = async(copy=false)=>{
    if(saveBusy || reviewBusy || !canStartRevision())return;saveBusy=true;
    try{
      await instructionQueue;await syncInstruction();await flushGeneralNote();await flushSectionNotes();
      const epoch=workEpoch;
      const displayed={correctionNote:correctionNote?.value || '',sectionNotes:displayedSectionNotes(),instruction:instruction?.value || '',pendingRevisionToken,proposalId,proposalStale};
      ...
      if(!snapshot || snapshot.correctionNote!==displayed.correctionNote || snapshot.instruction!==displayed.instruction ||
        ...
        displayed.proposalId && snapshot.proposalStale!==displayed.proposalStale ||
      ...
      workDocumentId=result.documentId;
      if(epoch===workEpoch){workDirty=false;workStatus.textContent='Saved · private local version '+result.version;}else workStatus.textContent='Newer edits are unsaved. Save again to retain them.';
      document.querySelector('[data-save-copy]').hidden=true;
    }catch(error){workStatus.textContent=error.message+' Local work kept.';document.querySelector('[data-save-copy]').hidden=false;}
  ...
  confirmDirtyNavigation=()=>{
    if(saveBusy || reviewBusy)return false;
