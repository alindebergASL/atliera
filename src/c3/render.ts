...
(() => {
  ...
  const dockInspector = () => Boolean(window.matchMedia?.('(min-width: 1440px)').matches && typeof evidenceDialog?.show === 'function');
  ...
  const inspectorRoutes = () => {
    ...
    if(back) { back.hidden = inspectorMode !== 'evidence' || !detailView; back.textContent = 'Back to ' + (detailView?.title || 'item'); }
  ...
  evidenceDialog?.addEventListener('keydown', event => { if(event.key === 'Escape' && evidenceDialog.getAttribute('data-docked') === 'true') {event.preventDefault(); evidenceDialog.close();} });
  ...
  document.addEventListener?.('click', (event) => {
    ...
    if (content && typeof evidenceDialog?.showModal === 'function') {
      ...
      evidenceDialog.querySelector('#evidence-panel-title').textContent = 'Evidence · ' + (link.getAttribute('data-context') || '') + ' · ' + target.querySelector('summary').textContent;
...
function home(frozen: FrozenC3AccountContext, hasDraft: boolean, recorded: boolean, revisionPending: boolean, topic?: ResearchTopic, selectedReading?: string): string {
  ...
  const tile = (note: AccountReading) => `<a data-detail-link aria-haspopup="dialog" data-detail-target="detail-${esc(note.id)}" class="priority-tile" href="${esc(researchUrl(readingTopic(note), note.id))}"><span class="item-icon">${uiIcon("priority")}</span><div class="priority-copy"><h3>${esc(note.title)}</h3><p>${esc(note.text)}</p>${note.limit ? `<p class="reading-limit">${esc(note.limit)}</p>` : ''}</div><span class="item-arrow" aria-hidden="true">›</span><span class="view-details">View details</span></a>`;
  const summarySection = (group: 'people' | 'technology', title: string, empty: string) => `<section id="account-${group}" class="summary-section"><h2>${title}</h2>${notes(group).slice(0, 2).map(note => `<a data-detail-link aria-haspopup="dialog" data-detail-target="detail-${esc(note.id)}" class="summary-row" href="${esc(researchUrl(group, note.id))}"><span class="item-icon">${uiIcon(group)}</span><div><h3>${esc(note.title)}</h3><p>${esc(compactReading(note).text)}</p><span class="view-details">View details</span></div><span class="item-arrow" aria-hidden="true">›</span></a>`).join('') || `<p>${empty}</p>`}<a class="quiet-link" href="${esc(researchUrl(group))}">Explore ${group === 'people' ? 'people &amp; operating context' : 'technology &amp; services'} →</a></section>`;
...
function workshop(state: Extract<C3PageState, { page: "workshop" }>): string {
  ...
  return `<main id="main" tabindex="-1"><h1>Workshop</h1><p class="lede">Prepare for a conversation and return to your account work.</p><div class="hero-actions"><a class="button" href="/?prepare=1">Prepare brief</a></div>${state.generation && !state.generation.available ? `<p class="context-note">${esc(state.generation.explanation)}</p>` : ''}<div class="workshop-groups">${state.work?.available ? `<section class="workshop-list summary-section"><div class="section-heading"><h2>Saved briefs</h2></div><p data-work-list-status role="status">${state.work.storageError ? 'Saved briefs could not be loaded. Current work is kept.' : ''}</p>${errorDiagnostics(state.work.storageError)}${state.work.savedWorks.map(item => `<article class="workshop-item"><h3>${esc(item.title || item.intendedOutcome || `Brief for ${item.audience}`)}</h3><p class="saved-metadata">${esc(item.audience)}${item.meetingDate ? ` · ${esc(humanDate(item.meetingDate))}` : ''}</p>${item.intendedOutcome && item.intendedOutcome !== (item.title || item.intendedOutcome || `Brief for ${item.audience}`) ? `<p>${esc(item.intendedOutcome)}</p>` : ''}<p class="meta">${originLabel(item.origin)} · Version ${item.version}${savedTime(item.savedAt) ? ` · Last saved ${savedTime(item.savedAt)}` : ' · Last-saved time not available'}</p><button type="button" class="secondary" data-reopen-work="${esc(item.documentId)}" data-replaces-unsaved="${Boolean(state.hasDraft && !state.work?.saved)}" data-work-version="${state.work?.workVersion ?? 0}">Reopen saved brief</button></article>`).join('') || (state.work.storageError ? '' : '<p>No saved briefs for this account.</p>')}</section>` : ''}<section class="workshop-list summary-section"><div class="section-heading"><h2>Current work</h2></div>${state.hasDraft ? `<article class="workshop-item"><h3>${esc(state.work?.title || 'Meeting brief')}${state.revisionPending ? ' · revision pending' : ''}</h3><p>The current session draft and its notes.</p><a class="quiet-link" href="/?draft=1">Reopen session draft →</a></article>` : '<p class="workshop-empty">No meeting brief yet. Start with the audience and outcome, or use an editable worksheet below.</p>'}${edited.map(brief => `<article class="workshop-item"><h3>${brief.kind === 'strategy' ? 'Strategy' : 'Next steps'} worksheet</h3><p>${esc(brief.audience || 'Audience not set')} · Session version ${brief.version}</p><a class="quiet-link" href="/?kind=${brief.kind}">Open worksheet →</a></article>`).join('')}</section></div><section class="account-section"><h2>Planning worksheets</h2><p class="meta">Editable templates · not AI-generated.</p><nav class="journey-nav" aria-label="Planning worksheets"><a href="/?kind=strategy">Strategy worksheet →</a><a href="/?kind=next-steps">Next-steps worksheet →</a></nav></section></main>`;
}
...
function accountDetailLibrary(context: FrozenC3AccountContext): string {
  ...
  const sourceInfo = (source: EvidenceDisplay['source']) => `<p class="meta">${esc(source.publisher)} · ${esc(source.entity.name)}</p><p class="meta">Published ${esc(humanDate(source.publicationDate))} · Event ${esc(humanDate(source.eventDate))} · Current through ${esc(humanDate(source.evidenceCurrentThrough))}</p>`;
  const details = projection.details.map(detail => {
    const note = projection.readings.find(note => note.id === detail.readingId)!;
    return `<section id="detail-${esc(note.id)}" data-detail-title="${esc(note.title)}"><div data-detail-content data-reading-id="${esc(note.id)}" data-reading-topic="${detailTopic(note)}"><p class="eyebrow">${note.kind === 'hypothesis' ? 'Hypothesis · unvalidated' : note.kind === 'interpretation' ? 'Interpretation · unreviewed' : 'Source summary · unreviewed'}</p>${note.period ? `<p class="meta">${esc(note.period)}</p>` : ''}${note.limit ? `<p class="reading-limit">${esc(note.limit)}</p>` : ''}${detail.sections.map(section => {
      const {source} = byEvidence.get(section.evidenceId)!;
      return `<section class="retained-detail" data-source-id="${esc(section.sourceId)}" data-evidence-id="${esc(section.evidenceId)}"><h3>${esc(section.title)}</h3><p class="meta">Exact retained passage</p><blockquote>${esc(section.exactText)}</blockquote>${sourceInfo(source)}<a class="source-chip" data-evidence-link data-context="${esc(note.title)}" data-support="Exact retained source context · inspection does not establish current status or approval" href="#detail-evidence-${esc(section.evidenceId)}">${uiIcon('source')}<span>Evidence · ${esc(source.title)}</span></a></section>`;
    }).join('')}${note.question ? `<section class="detail-question"><h3>Still to confirm</h3><p>${esc(note.question)}</p></section>` : ''}${context.context.declaredContradictions.map(text => `<p class="warning"><strong>Conflicting context.</strong> ${esc(text)}</p>`).join('')}<a class="quiet-link" data-open-research href="${esc(researchUrl(detailTopic(note), note.id))}">Open in Research →</a></div></section>`;
  }).join('');
...
function titleEditor(work: WorkDisplayState | undefined, fallback: string): string {
  return `<details class="title-editor"><summary>Edit title</summary><form data-title-form data-work-version="${work?.workVersion ?? 0}"><label for="work-title-input">Brief title</label><input id="work-title-input" name="title" data-title-input required maxlength="160" value="${esc(work?.title || fallback.slice(0,160))}"><button type="submit" class="secondary">Keep title</button><p data-title-status class="meta" role="status"></p>${errorDiagnostics()}</form></details>`;
}
