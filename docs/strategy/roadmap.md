...
- decision_implementation_work_authorized: `M4-public-http-fetch-v1-first-party-no-live-execution`
...
| **M4 — Evidence acquisition v1** (deterministic system-side fetcher as the first orchestrator-held MCP integration; allowlisted, sandboxed, provenance-tagged; adversarial injection corpus in definition of done; L0 only in this slice; **its own capstone, its own visible artifact and retro — not co-shipped with any M5 milestone**) | 🔶 in progress; implementation slice complete, not shipped | Anchors: `src/capability/public-http-fetch-policy.ts`, `src/capability/m4-public-http-fetch-mediation.ts`, and `fixtures/validation/m4-public-http-fetch-v1-recorded-proof.json`. The real registry/MCP/mediation path has a deterministic recorded/injected proof with zero live egress. Milestone completion still awaits later explicit live GO/execution, retro, and successor surface as appropriate. Draft packet: `docs/runbooks/m4-public-http-fetch-v1-status-and-fedex-live-packet.md`. No live acquisition or readiness claim. |
