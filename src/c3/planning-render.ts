...
export function sectionNoteEditor(title: string, recordId: string, notes: SectionNotes, pending: boolean): string {
  ...
  return `<div class="section-note"><p data-saved-copy class="user-copy"${text ? "" : " hidden"}>${text ? `User note · ${esc(text)}` : ""}</p><details><summary>Note or correction for ${esc(title)}</summary><p class="meta">User-authored session note. The recorded model text above stays unchanged. Only the exact recorded correction in Draft review can replay a revision.</p><form data-local-edit data-edit-key="meeting-${key}" data-endpoint="/api/section-note" data-record-id="${esc(recordId)}" data-section="${esc(title)}"><label for="note-${key}">Your note for ${esc(title)}</label><textarea id="note-${key}" name="text" maxlength="1000"${pending ? " disabled" : ""}>
