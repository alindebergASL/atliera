...
export function renderAccountResearch(items: readonly AccountResearchInspection[], selectedTopic?: AccountResearchTopic, inspector = false): string {
  ...
  // Keep public research outside selected draft evidence while sharing the same inspector.
  return rendered.replace(/<details id="research-excerpt-([^"<>]+)"([^>]+)><summary>([\s\S]*?)<\/summary>([\s\S]*?)<\/div><\/details>/gu,
    (_match, id: string, attributes: string, _summary: string, content: string) => {
      const item = visibleItems.find(item => esc(item.candidate.id) === id)!;
      const {candidate, source} = item;
      const title = candidate.topic === 'people' ? candidate.exactExcerpt.split('\n')[0]! : source.label;
      const specifics = candidate.topic === 'people' ? `<h3>Source-reported role</h3><p>${esc(candidate.exactExcerpt.split('\n').slice(1).join(' '))}</p>` : '';
      return `<article class="research-inspection-entry"><h3>${esc(title)}</h3><p>${esc(candidate.sourceAttributedSummary)}</p><a class="quiet-link" data-detail-link aria-haspopup="dialog" data-detail-target="research-detail-${id}" href="#research-excerpt-${id}">View details<span class="sr-only"> · ${esc(title)}</span></a></article><div hidden><section id="research-detail-${id}" data-detail-title="${esc(title)}"><div data-detail-content><p class="detail-orientation">${esc(candidate.sourceAttributedSummary)}</p>${specifics}<h3>What remains unclear</h3><p>${esc(candidate.limitation)}</p><p class="meta">Only a short public passage is retained for this subject. No expanded research or current status is established.</p><p class="meta">${esc(source.label)} · ${source.publicationDate ? esc(source.publicationDate) : 'Undated'}</p><a class="quiet-link" data-research-link data-context="${esc(title)}" data-support="Unreviewed public research · excluded from preparation and model inputs" href="#research-excerpt-${id}">View evidence</a></div></section><details id="research-excerpt-${id}"${attributes} data-source-title="${esc(source.label)}"><summary>Exact excerpt and source details</summary>${content}</div></details></div>`;
    });
}
