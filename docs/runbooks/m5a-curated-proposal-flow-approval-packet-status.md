...
`packet_artifact_id` uniqueness review (operator flag 2026-06-17, fixed pre-merge):

The first draft of step 2 shortened the packet ID from `m5a-approval-packet:${contractArtifactId}:${draftedBy}:${now}` to `m5a-pkt:${proposalSetId}:${draftedBy}:${now}` to fit `SAFE_ID`'s 121-char cap. **That shortening dropped `flow_id`, which is load-bearing for uniqueness.** Step 1 generates a unique `contract_artifact_id` per `(proposal_set_id, flow_id)` pair, so two contracts can exist over the same proposal_set_id but different flow_ids; under the shortened form, packets drafted over those two contracts with the same drafter and `now` would have shared a `packet_artifact_id` while their `references_contract_artifact_id` correctly differed. A downstream slice treating `packet_artifact_id` as a key (idempotency, lookup) would have conflated them. The contract reference triple stayed correct (both fields read from the same locked-in contract snapshot), but the ID surface had silently weakened.

The fix encodes the full `contractArtifactId` in the packet ID: `m5a-pkt:${contractArtifactId}:${draftedBy}:${now}`. This is ~111 chars on the committed fixture, under the 121-char cap, and preserves all the identity information the original concatenation carried. A regression locks: (a) two contracts with the same proposal_set_id but different flow_ids produce distinct packet_artifact_ids; (b) the packet_artifact_id encodes the contract_artifact_id verbatim (so the ID and the triple cannot point at different contracts); (c) the ID stays within the SAFE_ID cap on the committed fixture.

This is the "tighten a thing to make CI pass, accidentally loosen a safety property" pattern, caught pre-merge by the operator before tree-side review. Recorded here as a doctrine note: shortening identifiers to satisfy a validator is the kind of change that warrants explicit review of what got dropped, especially when the dropped field distinguishes references that downstream slices key on.
