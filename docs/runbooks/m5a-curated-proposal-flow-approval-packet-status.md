...
This runbook records the M5a step 2 artifact — a drafted-and-unarmed approval packet over the M5a step 1 contract. Step 3 now consumes the verified packet and produces a separate armed artifact; Step 4 is the still-future execution slice. Step 2 itself remains no-call and drafted-and-unarmed by default.
...
- M5a step 2 reuses the M3-shipped surfaces (Step 4 will use the M3 step 3a executor + M3 step 3b reader and render-composer); introduces no new mediation gate; adds no new acquisition risk class. Its `flow_constraints.mediation_gate_level: L0` is a prospective policy pin only, not an effect stamp. A `mediation_gate_level` stamp belongs only on a completed Step 4 effect. The packet is marker-closed against fresh provider call and system-side acquisition.
...
- **M5a Step 4 — flow execution — is next.** It must consume the armed artifact plus packet and contract, re-verify all three, enforce expiry at execution time, and atomically consume Step 3's stable one-shot key in the same durable transaction as the effect. It must refuse replay after consumption. Only a completed effect may stamp `mediation_gate_level: L0`.
