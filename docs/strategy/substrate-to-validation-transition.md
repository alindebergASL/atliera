...
Current status: `docs/runbooks/live-product-preview-status.md` now records the sanitized one-run live product preview execution at commit `344c89b`. The run used OpenRouter `owl-alpha` for `graph.propose`, exactly one representative account, exactly one provider call, and one Workshop render from the produced graph. Activation gates, credential status, provider call, response contract, cost ledger, full-pipeline packaging, bootstrap evidence verifier, and Workshop preview passed. The run recorded input tokens 817, output tokens 391, observed cost $0.00, no tools/plugins/web search, no production writes, and no runtime/model-mode integration. The Workshop preview path reported providerCallsMade 0 and productionWrites false. This is a bounded product-surface validation signal only; it does not imply launch readiness, product readiness, production readiness, broad model quality, multi-account readiness, provider comparison, corpus expansion, or provider lock-in. `docs/runbooks/live-product-preview-usefulness-gate.md` now defines the no-spend assessment gate: `assessLiveProductPreviewUsefulness(...)` consumes already-produced, already-sanitized one-run live product preview facts, preserves `launch_readiness_claim: false`, and authorizes no provider calls, no provider spend, no production writes, no runtime/model-mode integration, no comparison, no corpus expansion, no product-preview expansion, no web search, and no tools or plugins before any separate expansion approval packet. `docs/runbooks/live-product-preview-usefulness-assessment.md` applies that gate to preview ref `live-product-preview-20260528a` and records `weak-but-valid` with `insufficient_useful_lenses`; it keeps `approves_expansion_or_comparison: false`. `docs/runbooks/live-product-preview-usefulness-remediation.md` records the no-spend remediation plan for prompt contract, proposal schema, Workshop lens mapping, product-surface expectations, and fixture coverage while blocking live reruns, provider comparison, corpus expansion, product-preview expansion, and readiness claims. `docs/runbooks/live-product-preview-lens-diagnostic.md` then applies the narrower no-spend lens diagnostic and classifies the first preview as `structure-absent-account-limitation`: sanitized supported graph structure exists for Signals only, not Maps or Plays, so current-account remediation stops and prompt/schema work must not pressure unsupported Maps or Plays content. `docs/runbooks/owl-alpha-validation-framing.md` records the future `owl-alpha` validation framing: cost-limited: false and sample-limited-by-cost: false, so later `owl-alpha` batches should be sized by safety, provenance, quality, role coverage, and usefulness value rather than spend avoidance.

Current status: `docs/strategy/first-validation-cycle-exit.md` records the first validation cycle exit assessment. It chooses no-spend methodology codification followed by a narrow product-facing fake-mode runtime slice as the next phase, while preserving that the first validation cycle does not imply launch readiness, does not approve runtime/model-mode integration, and does not approve provider comparison or corpus expansion.
