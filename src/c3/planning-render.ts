...
export function sectionNoteEditor(title: string, recordId: string, notes: SectionNotes, pending: boolean): string {
  ...
  return `<div class="section-note"><p data-saved-copy class="user-copy">${text ? `User note · ${esc(text)}` : "No user note for this section."}</p><details><summary>Note or correction for ${esc(title)}</summary><p class="meta">User-authored session note. The recorded model text above stays unchanged. Only the exact recorded correction in Draft review can replay a revision.</p><form data-local-edit data-edit-key="meeting-${key}" data-endpoint="/api/section-note" data-record-id="${esc(recordId)}" data-section="${esc(title)}"><label for="note-${key}">Your note for ${esc(title)}</label><textarea id="note-${key}" name="text" maxlength="1000"${pending ? " disabled" : ""}>${esc(text)}</textarea><p data-local-status role="status"></p><div class="hero-actions"><button type="submit"${pending ? " disabled" : ""}>Keep section note</button><button type="button" class="secondary" data-local-cancel${pending ? " disabled" : ""}>Cancel edit</button></div></form></details></div>`;
}
...
export function planningPage(context: FrozenC3AccountContext, brief: PlanningBrief): string {
  ...
  const field = (name: string, label: string, value: string, max: number, required = false) => `<div class="field"><label for="plan-${name}">${label}</label><textarea id="plan-${name}" name="${name}" maxlength="${max}"${required ? " required" : ""}>${esc(value)}</textarea></div>`;
