...
- **The H3 retro input is now richer.** Step 2 imported the snapshot helpers from step 1 rather than hand-rolling a fifth site. The M5a layer is one consolidated site of the discipline. The H3 ratification, when it happens, should treat the M5a layer as one migration site of the four total, AND treat the M5a-step-1-to-step-2 export-and-import as evidence that small, namespace-local consolidations are easy when modules share a layer — informing Q4 specifically.
