...
The repository still does not claim launch readiness. The current no-spend Gate 3 foundation includes fake/local Workshop serving, local health, local durable DB boot/migration, local backup/restore, a local bearer auth seam, a plan-only lab deployment descriptor contract, a plan-only deployment-target healthcheck contract, a plan-only lab host supervision contract, and a plan-only lab backup policy contract. The next foundation-first decision is whether to continue with no-deploy lab wiring contracts or separately authorize a bounded lab deployment/probe slice, with any AWS service use kept behind portable adapter/config seams.
