...
This runbook records the M5a Step 1 artifact — a no-call typed contract for the curated-source proposal flow end-to-end. M5a is the doctrine-loop capstone on curated public sources (per `docs/strategy/roadmap.md` §P-track M5a and the M3 retro §4 disposition). The contract slice executes nothing. Step 2 consumes and validates this contract to produce the drafted packet; Step 3 produces a separate armed-state artifact; the fixture-bound Step 4 capstone now consumes and re-verifies all three while independently pinning the exact committed proposal digest.
...
- **Contract-only slice scope confirmed (Step 1).** Step 1 ships typed shape + builder/verifier + runbook + safety test + INDEX. Step 2 packet drafting and Step 3 recorded-flow arming now exist in their own modules; Step 4 execution remains separate. Path 1 still means M5a has no runtime-provider-call arming surface.
...
- Step-1 schema v2 carries `materialization_input_sha256`, calculated from a recursively bounded own-data snapshot and deterministic key-sorted canonical JSON. Proxy, accessor, symbol, exotic, cyclic, non-JSON, and over-budget shapes refuse without invoking getters. Legacy schema-v1 artifacts lack this binding and are not accepted as current.
- `canonicalM5aCuratedProposalFlowContractArtifactId(proposalSetId, accountId, flowId, materializationInputSha256)` derives `m5a-flow-contract:<40-hex-digest>` from an unambiguous JSON tuple and a 160-bit SHA-256 prefix. Both builder and verifier call it from validated locals. The materialization digest and account dimensions are load-bearing: source-byte or account changes rotate the contract ID and the downstream packet/arming/one-shot chain.
...
- **M5a Step 4 now exists as a fixture-bound capstone.** It consumes and re-verifies the contract, drafted packet, and armed-state artifact; requires the Step-1-bound materialization digest and supplied snapshotted input digest to match the exact committed-input SHA-256 pin; enforces execution-time expiry and one-shot consumption; and performs one local durable write, read-back, and Workshop render. It is not a generic approval framework and does not broaden this Step-1 contract's authority.
