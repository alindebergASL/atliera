...
| `workshop-public-proposal-durable-state-view-status.md` | active | Current read-only durable-state Workshop view over local durable DB graph snapshot rows; it renders M3 rows conservatively as Unverified plus pending-review decoration, surfaces rejected proposals only as non-graph audit context, and performs no provider calls, graph ingestion, durable writes, production writes, HTTP wiring, H-track work, or readiness claim. |
