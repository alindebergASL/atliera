...
Recorded mode prefills the exact initial audience, outcome, date, and duration from `prior/model-request.json`; the date never drifts with launch time. Home, Prepare, and Draft all say **Private candidate preview · Recorded responses · No live generation**, and distinguish any local replay wait from live timing. On the initial draft, “Exact correction available for the recorded revision” discloses copyable operator-recorded correction text. “Use exact recorded correction” deliberately copies those exact bytes into the textarea; it never runs automatically or silently replaces user text. “Revise brief” uses the real `/api/revise` then `/api/generate` paths behind one user action when setup is unchanged, updating generated regions without replacing the working document or its note editors. The older Prepare detour is no longer the enhanced-browser path; non-JavaScript forms retain their safe fallback. Arbitrary notes remain session notes, but only the exact recorded correction and prior draft identity match the recorded revision. No additional recorded response exists after that revision.
...
## Connected brief and unchanged actions

Home, Prepare, and Draft share account identity and route navigation. The draft shows the audience, intended outcome, date, and duration from its actual generation record, with unchanged model prose arranged as situation, opening, three ordered questions, and useful close. The proposed objective and technical session details are disclosed on demand. Historical initial recordings retain their explicit not-recommended warning, including while a revision is pending. Recorded revisions expose note keeping but disable further revision in the UI because no additional recording exists; exact replay refusal remains enforced in the provider.

A citation opens its native evidence disclosure in one interaction and adds a return link to the invoking passage. Exact excerpt, source URL, publisher, and publication/event/current-through dates appear together; full retained bounded clean text is one further disclosure. Fragment navigation stays in the document, preserving unsaved notes. Home evidence now includes source links, retained context, and the existing funding-row/header caveat. Account gaps and conflicts remain visible. These are display changes only; context, request, validator, and raw recording identities are unchanged.

Keeping a note updates the live status in place, preserving scroll, focus, open evidence, and any newer unsaved typing. Empty notes can deliberately clear a saved note. An exactly unchanged note reports that it was already kept. Empty or whitespace-only revision instructions, or instructions exactly equal to the correction already applied to the current revision, do not start a revision or mutate the draft/note. A saved note that has not yet been applied can still request a revision. Review identity and active/pending guards run before these checks.

Submitting meeting inputs exactly equal to the current successful record, with no active work or pending revision, reopens that record and preserves its note. The response visibly says no new generation occurred; provider counters do not increment. This compares all four validated input fields without fuzzy matching, normalization, historical-record search, or arbitrary replay fallback. Changed valid inputs and pending revisions retain the normal provider and cancellation paths; invalid inputs remain refused.
