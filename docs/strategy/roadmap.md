...
- decision_implementation_work_authorized: `M5b-FedEx-Gate-B-unarmed-one-shot-executor (implementation complete; no private execution or effects)`
- decision_implementation_start_condition: `exact base commit 81661693bd0c858a4e0c9400ff68c28cb0b277f3 and tree e40ff4b3d1a0c394145b9b63ddb5efeaab785a5e`
...
- next_recommended_work: `no private read authorized; PR #289 Gate A is merged and Gate B implementation is unarmed; a future private read requires a fresh explicit external one-shot GO with a fixed ten-minute maximum lifetime and the exact path/identity/base/CI/deadline bindings`

Attempt 2 consumed its separately granted one-shot live authority. Attempts 1 and 2 are both permanently consumed. PR #289 M5b Gate A is merged. Gate B implementation is complete as an unarmed candidate; no private execution occurred, and current effective authority is `none`.
...
### M5b Gate B implementation in progress: unarmed FedEx one-shot candidate

PR #289 merged the repaired Gate A pre-effect path at `src/workshop/m5b-fedex-system-acquired-source.ts`, `src/workshop/m5b-fedex-review-composition.ts`, and `src/workshop/m5b-fedex-prewrite-workshop.ts`. Gate B now adds the internal exact-purpose future core at `src/workshop/m5b-fedex-gate-b-unarmed-executor.ts`, the deliberately non-executable approval template at `fixtures/validation/m5b-fedex-gate-b-authorization-template.json`, and its status contract at `docs/runbooks/m5b-fedex-gate-b-unarmed-executor-status.md`. The core is not exported from `src/index.ts`; there is no default authorization or custody path, caller repository/replay root, production wrapper, current authorization artifact, or current invocation.

The unarmed core binds base commit `81661693bd0c858a4e0c9400ff68c28cb0b277f3`, tree `e40ff4b3d1a0c394145b9b63ddb5efeaab785a5e`, successful CI run `29435522041`, and the exact M4 production pins. A future fresh external private GO must supply the canonical mode-`0600` custody path, set `validUntil` no more than ten minutes (`600000` ms) after `authorizedAt`, and expire no later than `2026-08-13T18:41:11.277Z` absent separate retention ratification. A later reviewed arming wrapper must literally pin that GO's exact-byte digest, the reviewed executor commit/tree and executable seal, expected owners, and one fixed trusted artifact-independent replay root; the GO cannot select replay storage or create trust. The core refuses path/file/pin/identity/replay/time drift, durably consumes before its single read, retries zero times, and returns only sanitized production-admitted, unratified, unverified in-memory outputs. The committed synthetic path cannot consume authorization and writes exactly five scoped deterministic outputs. The predecessor Gate A count of three is historical only. M5b is **🔶 in progress, Gate B unarmed, not shipped**. `current_effective_authorization` is `none`; private reads, product provider calls, acquisitions, graph/DB reads and writes, deployments/AWS actions, retries, and external/product effects are all 0. A future private read requires a fresh explicit GO and the separate reviewed arming wrapper.
...
| **M5b — Does-its-job-once, system-acquired sources** (the original capstone framing carried forward from the frozen big-picture review: pick one real account → system fetches public sources via M4 → loop runs → Workshop renders durable state with honest trust labels; visible artifact: a shareable Workshop account page about a real company, every claim traceable to a stored source, every unverified item labeled) | 🔶 in progress — Gate B implementation unarmed, not shipped | PR #289 Gate A is merged. The internal Gate B candidate has no barrel/package/private execution route, default/private path, caller replay root, or current authorization. A later reviewed wrapper must literally pin the exact GO digest, executor commit/tree and executable seal, owners, and fixed replay root. The core durably consumes before one read, retries zero times, and sanitizes unratified/unverified outputs. It was not invoked against custody. External/product effects 0; the active Gate B synthetic generator writes 5 local outputs. Historical Gate A wrote 3. A future private read requires a fresh explicit GO and separate reviewed arming wrapper. Historical predecessor status, now superseded: 🔶 in progress — Gate A pre-effect, unarmed, not shipped. |
...
| **H4 — Single guarded execution entrypoint** | ⬜ not started; not next-up | One general chokepoint so future executions cannot skip a check. The M5b-only Gate B executor is an exact-purpose boundary, not H4 or a general framework. Any future H4 work requires a separate explicit operator decision; any M5b private read requires its own fresh explicit external GO. |
...
7. **Post-M5a product direction** — **RESOLVED THROUGH THE UNARMED M5b GATE B IMPLEMENTATION FRONTIER: H2 NO-NETWORK ECHO PROOF → THIN M4 `public_http_fetch_v1` → M5b FEDEX GATE A REVIEW DRAFT → EXACT-PURPOSE GATE B EXECUTOR.** H2 shipped at PR #284. M4 completed one bounded acquisition and its visible closeout; both attempt authorities are permanently consumed. PR #289 Gate A is merged. Gate B supplies only an unarmed exact-purpose one-shot boundary and synthetic proof; it did not read private custody. Public templates and hashes are not authority or human decisions. A future private read requires a fresh explicit external GO with the exact path, identity, base/tree, CI, zero-retry/one-read, and deadline bindings. Provider/model execution and every later effect remain separately gated.
