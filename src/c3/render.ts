...
(() => {
  ...
  reviewForm?.addEventListener('submit', async (event) => {
    ...
    try { const payload = await requestJson('/api/note', { note: data.get('note'), recordId }); if (token === reviewToken && typeof payload.html === 'string') replacePage(payload); }
