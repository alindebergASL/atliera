...
The entire H-track (H1–H5 and A3) was **frozen until M3 ships**. **The freeze was lifted at the M3 retro (`docs/reviews/m3-retro.md`, PR #275).** That retro's H3-first and H1-with-M5a recommendations remain frozen historical evidence in that file; they are superseded for current sequencing by the operator ratification in `docs/reviews/m5a-step4-before-h3-ratification.md`. H3's plan is complete and merged through PR #277, but H3 implementation is queued until after the M5a capstone and M5a retro unless a concrete Step-4 safety blocker requires H3. H1 is not a prerequisite for, or paired with, M5a Step 4. The post-capstone M5a retro must re-evaluate the H1/H2/H3 implementation order against then-current evidence; H4, H5, and A3 remain not next-up.

### Current ratified implementation slice — M5a Step 4

The operator ratified M5a Step 4 as the next implementation slice, before H3 implementation unless a concrete safety blocker emerges. M5a's shared helpers are already hardened across Steps 1–3; cross-layer H3 consolidation immediately before the capstone would enlarge the regression surface without proving more product value. The durable decision and the distinction between the earlier agent recommendation and operator ratification are recorded in `docs/reviews/m5a-step4-before-h3-ratification.md`.

Step 4's implementation boundary is exactly:
...
This sequencing decision authorizes implementation work only. It does not consume an arming, execute the flow, write or read durable state, render an artifact, or otherwise execute an effect.
...
| **H3 — Snapshot-primitive consolidation + negative-control automation** | ⬜ implementation not started; plan complete/merged | Plan merged through PR #277 (`a879c11`). Implementation is queued until after the M5a capstone and M5a retro unless a concrete Step-4 safety blocker requires it. The plan remains evidence for the eventual cross-layer migration, not authority to put it ahead of Step 4. |
...
6. **M5a Step 4 vs. H3 implementation** — **RESOLVED: STEP 4 FIRST.** Steps 1–3 are merged; Step 4 proceeds before H3 implementation unless a concrete safety blocker emerges. H3's plan is complete/merged, while implementation is queued until after the M5a capstone and M5a retro. See `docs/reviews/m5a-step4-before-h3-ratification.md`.
