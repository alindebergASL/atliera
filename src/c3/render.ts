...
(() => {
  ...
  const displayedContext = document.querySelector('meta[name="c3-context"]')?.getAttribute('content') || '';
  ...
  const requestJson = async (url, body, signal) => {
    const response = await fetch(accountUrl(url), { method: 'POST', headers: { 'content-type': 'application/json', 'x-c3-csrf': csrf, 'x-c3-account': account, 'x-c3-document': workDocumentId, 'x-c3-context': displayedContext }, body: JSON.stringify(body), signal });
  ...
  const dockInspector = () => {
    ...
    const sidebar = parseFloat(style.getPropertyValue('--atl-sidebar')) || 0;
...
function home(frozen: FrozenC3AccountContext, hasDraft: boolean, recorded: boolean, revisionPending: boolean, topic?: ResearchTopic, selectedReading?: string, work?: WorkDisplayState): string {
  ...
  const retainedRows = (compact = false) => (compact ? projection.retainedSections.slice(0, 3) : projection.retainedSections).map(item => `<article class="account-reading retained-proposal" id="reading-${esc(item.id)}" tabindex="-1"${selectedReading === item.id ? ' data-selected="true"' : ''}><h3>${esc(item.title)}</h3><p class="user-copy">${esc(item.text)}</p><p class="meta">Original retained proposal · ${esc(item.state)} · unreviewed presentation</p><p class="support">${refs(item.evidenceIds, item.title)}</p></article>`).join('');
  const retainedContext = projection.retainedSections.length ? `<section class="account-section" id="retained-proposal-sections"><h2>Retained proposal context</h2><p class="section-intro">Existing proposal sections with their original status and evidence. Reported developments do not establish change since an earlier review; interpretations remain proposed.</p>${retainedRows(Boolean(!topic))}${!topic ? `<a class="quiet-link" href="${esc(researchUrl('initiatives'))}">Read all retained sections →</a>` : ''}</section>` : '';
  ...
  if (topic) {
    ...
    const selectionNotice = selectedReading && !selected && !projection.retainedSections.some(item => item.id === selectedReading) ? '<p class="context-note" role="status">That reading is not available in this topic’s retained account evidence. Explore the available research below.</p>' : '';
...
function savedWorkRows(work: WorkDisplayState | undefined, hasDraft: boolean): string {
  const savedWorks = work?.savedWorks ?? [];
  // Extend the real ID suffix until it distinguishes every document in this list.
  const shortId = (id: string): string => {
    let length = Math.min(8, id.length);
    while (length < id.length && savedWorks.some(item => item.documentId !== id && item.documentId.slice(-length) === id.slice(-length))) length++;
    return id.slice(-length);
  };
  const rowTitle = (item: WorkDisplayState['savedWorks'][number]) => displayWorkTitle(item.title, item.intendedOutcome || `Brief for ${item.audience}`);
  return savedWorks.map(item => {
    const current = Boolean(hasDraft && item.documentId === work?.documentId);
    const currentVersion = current && item.version === work?.version;
    const title = rowTitle(item);
    const identity = `Record ${shortId(item.documentId)}`;
    const collision = savedWorks.some(other => other.documentId !== item.documentId && rowTitle(other) === title);
    const openLabel = collision ? ` aria-label="${esc(`${title} · ${identity}`)}"` : '';
    const opener = currentVersion && work?.saved ? `<a class="saved-title" href="/?draft=1"${openLabel}>${esc(title)}</a>` : `<button type="button" class="saved-title"${openLabel} data-reopen-work="${esc(item.documentId)}" data-replaces-unsaved="${Boolean(hasDraft && !work?.saved)}" data-work-version="${work?.workVersion ?? 0}">${esc(title)}</button>`;
    return `<article class="workshop-item"${current ? ' data-current-work="true"' : ''}><div><h3>${opener}</h3>${item.intendedOutcome && item.intendedOutcome !== title ? `<p class="work-preview">${esc(item.intendedOutcome)}</p>` : ''}${collision ? `<p class="saved-metadata" data-saved-identity>${esc(identity)}</p>` : ''}<p class="saved-metadata">${esc(item.audience)}${item.meetingDate ? ` · Meeting ${esc(humanDate(item.meetingDate))}` : ''}</p><p class="meta">${originLabel(item.origin)} · Version ${item.version}${savedTime(item.savedAt) ? ` · Last saved ${savedTime(item.savedAt)}` : ''}${current ? ` · ${!currentVersion ? `Current session is based on saved version ${work?.version}` : work?.saved ? 'Current brief' : 'Current session has unsaved changes'}` : ''}</p></div><details class="saved-record-details"><summary>Brief details</summary><p class="meta">${esc(identity)}</p><p class="meta">Open the brief, then Document details to edit its title.</p></details></article>`;
  }).join('');
}
...
function prepare(context: FrozenC3AccountContext, request: C3MeetingFormState, error: string | undefined, hasDraft: boolean,
  ...
  const pending = revisionPending && correctionNote !== undefined && correctionNote.length > 0 ? `<p class="context-note"><strong>Revision pending — previous draft preserved.</strong> Cancel stops generation; reopen the draft to deliberately discard the revision.</p><details class="recorded-note"><summary>Correction included in this revision</summary><p>This exact correction note remains session-only. It is not approval or account truth. Cancel stops generation but does not discard the pending revision.</p><pre>${esc(correctionNote)}</pre></details>` : "";
...
function draftPage(context: FrozenC3AccountContext, record: C3GenerationRecord, note: string, recordedCorrection: string | undefined,
  ...
  const recordedHelp = state?.revisionUnavailableReason !== undefined ? `<p class="meta replay-disclosure" id="replay-instruction-help">${esc(state.revisionUnavailableReason)}</p>` : recordedCorrection === undefined ? "" : syntheticPreview ? (record.revision === null
...
function preparationContext(context:FrozenC3AccountContext):string {
  const target = context.context.directResearch;
  const fresh = target ? context.context.admittedSources.find(source => source.excerpts.some(excerpt => excerpt.evidenceId === target.selectedEvidenceId)) : undefined;
  const targeted = target && fresh ? `<section data-targeted-context><h3>Selected fresh source findings</h3><p>Agent-proposed documentary statements · validated evidence linkage · no owner approval. This new brief must use the selected passage.</p><p>${fresh.excerpts.length} new admitted passages from 1 direct source; ${context.context.admittedSources.length - 1} historical admitted sources retained.</p>${fresh.excerpts.map(excerpt => `<details${excerpt.evidenceId === target.selectedEvidenceId ? ' open' : ''}><summary>${excerpt.evidenceId === target.selectedEvidenceId ? 'Selected finding' : 'Related source finding'}</summary><blockquote>${esc(excerpt.exactExcerpt)}</blockquote><p class="meta">${esc(fresh.publisher)} · Retrieved ${esc(fresh.retrievedAt)}. Publication, event and current-through dates: unknown.</p><p class="research-identity">Original source ${esc(target.sourceId)} · Snapshot ${esc(target.run.snapshotId!)}<br>Derived evidence ${esc(excerpt.evidenceId)} · Positions [${excerpt.sourceCharStart}, ${excerpt.sourceCharEnd}) · SHA-256 ${esc(excerpt.exactExcerptSha256)}</p></details>`).join('')}<details><summary>Full retained source context</summary><pre class="research-clean-text">${esc(fresh.fullBoundedCleanText)}</pre></details><p>Conditional source statements do not establish present access or service availability. Page dates are not automatically publication dates. The selection is session-only until generated work is explicitly saved.</p></section>` : '';
