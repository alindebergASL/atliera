...
The first S3-compatible validation harness is `validateS3ArtifactStoreCompatibility({ client, bucket, prefix, probeId })`. It exercises an injected real client or high-fidelity emulator wrapper through the `S3ArtifactStore` boundary and reports deterministic sanitized results for text roundtrip, missing-object not-found behavior, last-write-wins overwrite behavior, prefix isolation, and max-payload refusal. The harness is not deployment wiring: it does not import storage SDKs, read env, construct clients, open sockets itself, choose buckets/endpoints/regions/credentials, or clean up objects. The caller owns a run-scoped probe namespace, backend/client construction, emulator fidelity notes, retention, and cleanup policy.
