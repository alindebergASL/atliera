...
**Assurance scope (v3).** Candidate-only evidence cannot satisfy ceremony. The workflow resolves actual GitHub reviews, validates pinned principal login/id/type tuples, reduces approvals and later rejections/dismissals in timestamp-and-event-id order, checks exact-head and explicit proposal bindings, and recomputes the proposal digest. Review edits trigger reevaluation. Its unavoidable trust limit is credential provenance: GitHub identifies the account, not whether its human owner or a delegated agent operated the credential.
