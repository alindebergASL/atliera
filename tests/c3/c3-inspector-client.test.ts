...
function inspector(wide=false, rootSize?:number, width=1440, readingWidth?:number){
 ...
 class Node {
  ...
  cloneNode(){return this;}prepend(...children:any[]){this.children.unshift(...children);}replaceChildren(...children:any[]){this.children=children;}
 ...
 const detail=new Node(),detailContent=new Node(),detailLink=new Node();detail.attrs={'data-detail-title':'CHPC · research computing'};detail.nodes={'[data-detail-content]':detailContent,dialog};detailLink.attrs={'data-detail-target':'detail-chpc',href:'/?view=research&topic=people&reading=chpc'};detailLink.parents={'a[data-detail-link]':detailLink};
 const evidence=new Node(),evidenceContent=new Node(),summary=new Node(),citation=new Node();summary.textContent='Retained operating scope';evidence.nodes={'[data-evidence-content]':evidenceContent,summary};citation.attrs={href:'#detail-evidence-1','data-context':'CHPC','data-support':'Related source context'};citation.parents={'a[data-evidence-link], a[data-research-link]':citation,'[data-detail-content]':detailContent,dialog};
 const secondDetailLink=new Node();secondDetailLink.attrs={...detailLink.attrs};secondDetailLink.parents={'a[data-detail-link]':secondDetailLink};
 ...
 const document={createElement:()=>new Node(),body:{style:{overflow:'auto'}},get activeElement(){return active;},querySelector:(selector:string)=>selector==='[data-evidence-dialog]'?dialog:selector==='#detail-evidence-1'?evidence:selector==='main'&&readingWidth?{getBoundingClientRect:()=>({width:readingWidth})}:null,getElementById:(id:string)=>id==='detail-chpc'?detail:null,addEventListener:(name:string,fn:(event:any)=>void)=>{handlers[name]=[...(handlers[name]??[]),fn];}};
 const window={innerWidth:width,...(rootSize ? {getComputedStyle:()=>({fontSize:String(rootSize),getPropertyValue:(key:string)=>key==='--atl-sidebar'?'208px':'420px'})} : {}),scrollX:0,scrollY:620,matchMedia:()=>({matches:wide}),scrollTo:(...position:number[])=>{pageScroll=position;},addEventListener:(name:string,fn:()=>void)=>{changes[name]=fn;},setTimeout:(fn:()=>void)=>fn()};
 ...
 return {dialog,panel,back,close,revisionReturn,revision,title,detailContent,detailLink,secondDetailLink,citation,direct,click,active:()=>active,pageScroll:()=>pageScroll,body:document.body,returns:()=>revisionReturns,resize(value:boolean){wide=value;changes.resize?.();}};
}
test('detail → citation → Back restores the exact retained detail, focus and independent scroll',()=>{
 ...
 ui.panel.scrollTop=240;ui.click(ui.citation);assert.equal(ui.back.hidden,false);assert.equal(ui.back.textContent,'Back to details');assert.equal(ui.panel.scrollTop,0);assert.equal(ui.title.textContent,'Evidence');assert.equal(ui.panel.children[0].textContent,'Retained operating scope');assert.equal(ui.close.textContent,'Close evidence');
 ui.back.click();assert.equal(ui.panel.children[0],ui.detailContent);assert.equal(ui.panel.scrollTop,240);assert.equal(ui.close.textContent,'Close details');assert.equal(ui.active(),ui.citation);
...
test('reservation requires 640px useful reading width and grows that minimum with enlarged text',()=>{
 const readable=inspector(true,16,1440);readable.click(readable.detailLink);assert.equal(readable.dialog.getAttribute('data-docked'),'true');
 const narrow=inspector(true,16,1280);narrow.click(narrow.detailLink);assert.equal(narrow.dialog.getAttribute('data-docked'),null);assert.equal(narrow.body.style.overflow,'hidden');
 const capped=inspector(true,16,1440,620);capped.click(capped.detailLink);assert.equal(capped.dialog.getAttribute('data-docked'),null,'A CSS reading-column cap cannot be ignored');
 const enlarged=inspector(true,24,1440);enlarged.click(enlarged.detailLink);assert.equal(enlarged.dialog.getAttribute('data-docked'),null);assert.equal(enlarged.body.style.overflow,'hidden');
});

test('changing the inspected subject from another page entry updates the Close focus receipt',()=>{
 const ui=inspector(true);ui.click(ui.detailLink);ui.click(ui.secondDetailLink);ui.click(ui.citation);ui.back.click();ui.close.click();
 assert.equal(ui.active(),ui.secondDetailLink);assert.deepEqual(ui.pageScroll(),[0,620]);
});
