...
- nginx/PM2/Certbot deployment plan has a plan-only descriptor contract, execution preflight contract, inert bounded execution approval packet, slice A execution status, and slice B backup/restore proof status; persistent deployment wiring, nginx/PM2/Certbot install, and domain/TLS work remain future work.
