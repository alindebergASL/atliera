...
The package builder and renderers are internal deterministic stages of the prepare path, not independent provenance authenticators. Both formatters require the complete serialized source-pack/candidate/review-packet triad. Shared admission first creates a bounded strict-JSON snapshot, then fully rehydrates the validated candidate, recomputes its SHA-256, validates the packet and source-pack schemas and topology, and cross-checks the candidate subject, exact private-builder record shape, mirrored graph-source identities and excerpt text, and embedded package/source-pack bindings. It returns only detached, deeply frozen snapshot-derived artifacts. Those checks establish package self-consistency and reject cross-artifact drift, post-admission caller mutation, or non-builder candidate semantics even when the affected candidate and packet self-hashes are recomputed. They still cannot prove excerpt membership in omitted original source bytes, distinguish a coherently fabricated triad from trusted preparation, or authenticate caller-fabricated production custody. Only `prepareM5bProductReview` performs that stronger check against admitted source bytes and opaque effect-ledger state. Product copy therefore calls exact excerpts **package-attributed**, not independently source-proven. Self-consistent rehashing is not a trusted provenance root; external review must pin the exact prepare result and, when execution provenance matters, its immutable execution receipt.
