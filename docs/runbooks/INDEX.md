...
- next recommended work: no M5b private read is authorized; PR #289 Gate A is merged and the Gate B internal core is unarmed, so any future private read requires a fresh explicit GO whose fixed maximum lifetime is ten minutes (`600000` ms), plus a separately reviewed arming wrapper that pins its exact bytes, the reviewed executor/seal, expected owners, and one fixed trusted replay root
- ratified next bounded implementation: none
- M4 is shipped upon closeout merge; M4 implementation authority has returned to none; PR #289 Gate A is merged; M5b Gate B implementation is an unarmed candidate and current effective authorization remains none
...
Per-artifact “next recommended work” sections record the local historical follow-up at the time each artifact closed; they do not override this index or the roadmap. PR #289 Gate A is merged and its post-merge CI identity is pinned by the unarmed Gate B implementation. No private read is currently authorized. A future read requires a fresh explicit GO supplied as an external private one-shot artifact with the exact custody path, identities, zero-retry/one-read contract, `validUntil` no more than ten minutes after `authorizedAt`, and an expiry no later than the original deadline unless a separately ratified bounded retention decision already exists. Provider, model, external ratification, write, acquisition, and deployment decisions remain separate.
...
| `m5b-fedex-system-acquired-pre-effect-gate-a-status.md` | superseded | Historical M5b Gate A unarmed pre-effect repair for the FedEx source-backed account snapshot: proportional hostile-input bounds; simulated fixture origin/source type; exact candidate-bound unratified review and retention drafts; exact filing-only model citations with no call; and future composition that re-admits caller-supplied custody bytes, rebuilds the production pack, and requires exact equality. Serialized public constants and caller-mintable hashes are identity, never authority or human ratification. The committed fixture preserves two Maps, empty Signals/Plays, and cannot compose with fixture bytes. At that historical boundary M5b is 🔶 in progress, not shipped, and its deterministic generator wrote exactly 3 local fixture outputs. Gate B is the active M5b scope. |
| `m5b-fedex-gate-b-unarmed-executor-status.md` | active | Post-PR-#289 unarmed Gate B internal exact-purpose core and non-executable approval template. There is no barrel/package/private execution route. A later reviewed arming wrapper must literally pin the exact GO digest, reviewed executor commit/tree and executable seal, expected owners, and one fixed trusted replay root. The GO cannot select replay storage. The core consumes durably before one read, retries zero times, returns sanitized unratified/unverified outputs, and has no current invocation or authority. The synthetic generator writes exactly 5 scoped local outputs; all external/product effect counters remain 0. |
...
| `own-data-snapshot-h3-slice-plan-status.md` | active | Historical/current H3 plan-only reference for consolidating the util.types.isProxy + own-data descriptor-snapshot + (selectively) deep-freeze discipline from three independent call sites (executor PR #271 / 91b7064, reader PR #274 / b2b7a09, render-side composer PR #274 / b2b7a09 with the three hardening rounds 5ed3762 → f281ac0 → 1150dc7 → f895fb4) into src/safety/own-data-snapshot.ts; it introduces no source module or runtime behavior. The prior M5a-vs-M4 sequencing question is resolved by the completed M4 closeout; H3 implementation remains unstarted, not next-up and unauthorized without a separate future operator decision. |
