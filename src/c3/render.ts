...
export type C3PageState =
  | { readonly page: "home" }
  | { readonly page: "prepare"; readonly request: C3MeetingFormState; readonly error?: string }
...
(() => {
  ...
  const replacePage = (html) => { document.open(); document.write(html); document.close(); };
  ...
  if (form) {
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (token === requestToken && typeof payload.html === 'string') replacePage(payload.html);
      } catch (error) {
  ...
  document.querySelector('[data-note-form]')?.addEventListener('submit', async (event) => {
    ...
    try { const payload = await requestJson('/api/note', { note: data.get('note') }); if (typeof payload.html === 'string') replacePage(payload.html); }
    catch (error) { if (status) status.textContent = error instanceof Error ? error.message : 'Could not keep correction note'; }
  });
  document.querySelector('[data-revise]')?.addEventListener('click', async () => {
    ...
    try { const payload = await requestJson('/api/revise', { note: note?.value || '' }); if (typeof payload.html === 'string') replacePage(payload.html); }
    catch (error) { if (status) status.textContent = error instanceof Error ? error.message : 'Could not request revision'; }
  });
})();`;
...
function home(context: FrozenC3AccountContext): string {
...
function evidenceLinks(refs: readonly string[], numberById: ReadonlyMap<string, number>): string {
  if (refs.length === 0) return "No evidence asserted";
  return refs.map((id) => `<a href="#evidence-${String(numberById.get(id) ?? 0)}">Evidence ${String(numberById.get(id) ?? "")}</a>`).join(" · ");
}
...
function draftPage(context: FrozenC3AccountContext, record: C3GenerationRecord, note: string): string {
  ...
  const supported = (title: string, item: C3SupportedText) => `<section class="draft-section"><p class="eyebrow">${esc(title)}</p><p>${esc(item.text)}</p><p class="support">${esc(supportLabel(item))} · ${evidenceLinks(item.evidenceRefs, numberById)}</p></section>`;
  return `<main><div class="draft-head"><div><p class="eyebrow">Meeting draft</p><h1>${esc(context.context.account.accountName)}</h1><p class="lede">${esc(draft.objective.text)}</p><p class="support">${esc(supportLabel(draft.objective))} · ${evidenceLinks(draft.objective.evidenceRefs, numberById)}</p></div><span class="badge">Proposed · Not reviewed · Not durably saved</span></div><div class="draft-grid"><div>${supported("Audience thesis", draft.audienceThesis)}${supported("Opening", draft.opening)}<section class="draft-section"><p class="eyebrow">Questions in order</p><ol class="questions">${draft.questions.map((question) => `<li><strong>${esc(question.question)}</strong><p class="learning">Learn: ${esc(question.intendedLearning)}</p><p class="support">Open question${question.evidenceRefs.length === 0 ? " · No evidence asserted" : ` · Related evidence context: ${evidenceLinks(question.evidenceRefs, numberById)}`}</p></li>`).join("")}</ol></section>${supported("Close criterion", draft.closeCriterion)}</div><aside><section class="draft-section"><p class="eyebrow">Risks & unknowns</p>${draft.risksUnknowns.map((item) => `<p>${esc(item.text)}</p><p class="support">${esc(supportLabel(item))} · ${evidenceLinks(item.evidenceRefs, numberById)}</p>`).join("")}${draft.warnings.map((warning) => `<div class="warning">${esc(warning.message)}${warning.evidenceRefs.length === 0 ? "" : `<br>${evidenceLinks(warning.evidenceRefs, numberById)}`}</div>`).join("")}</section><section class="evidence-list"><p class="eyebrow">Selected evidence</p>${draft.selectedEvidenceRefs.map((id, index) => { const item=sourceByEvidence.get(id)!; return `<details id="evidence-${String(index+1)}"><summary>Evidence ${String(index+1)} · ${esc(item.source.title)}</summary><blockquote>${esc(item.excerpt.exactExcerpt)}</blockquote><p class="meta">${esc(item.source.publisher)} · Published ${esc(humanDate(item.source.publicationDate))} · Event ${esc(humanDate(item.source.eventDate))} · Current through ${esc(humanDate(item.source.evidenceCurrentThrough))}</p><p class="meta"><a href="${esc(item.source.canonicalUrl)}" rel="noreferrer">Open source</a></p></details>`; }).join("")}</section></aside></div><section class="review"><p class="eyebrow">Review interaction prototype</p><h2>Inspect, note a correction, or request revision</h2><p>This bounded note stays in this server session. A revision receives it and the exact prior raw/draft identity; neither action changes account truth or supplies authenticated approval/C4 persistence.</p><form data-note-form><label for="correction-note">Correction for the next proposed revision</label><textarea id="correction-note" data-correction-note name="note" maxlength="1000" placeholder="Describe what should change">${esc(note)}</textarea><p class="status-line" data-review-status role="status" aria-live="polite"></p><button type="submit">Keep note in session</button><button class="secondary" type="button" data-revise>Request revision</button></form></section><p class="boundary">Temporal outcome: ${esc(draft.temporalOutcome.replace(/_/gu," "))}. Generated bytes are frozen for reproduction; that identity is not ratification or authenticated approval.</p></main>`;
}

export function renderC3Page(context: FrozenC3AccountContext, state: C3PageState, csrf: string): string {
  if (state.page === "home") return shell(context.context.account.accountName, home(context), csrf);
  if (state.page === "prepare") return shell(`Prepare for ${context.context.account.accountName}`, prepare(context, state.request, state.error), csrf);
