...
const ASSESSMENT = join(ROOT, "runtime-model-only-product-preview-runtime-smoke-usefulness-assessment.md");
...
function assertNoPrivateLeakage(label: string, text: string): void {
  for (const pattern of privateLeakagePatterns) {
...
function assertNoApprovalContradictions(label: string, text: string): void {
  for (const pattern of forbiddenApprovalContradictions) {
    assert.doesNotMatch(text, pattern, `${label} broadened approval with ${pattern}`);
  }
}

test("runtime-smoke tiny expansion approval is docs-only and tied to the useful no-spend assessment", () => {
  const doc = read(APPROVAL);

  for (const required of [
    /Runtime Model-Only Product-Preview Runtime Smoke Tiny Expansion Approval Packet/i,
    /Status: pre-run docs-only approval packet\. This PR does not execute a provider call\./i,
    /runtime-model-only-product-preview-runtime-smoke-usefulness-assessment\.md/i,
    /runtime-model-only-product-preview-runtime-smoke-corrected-retry-status\.md/i,
    /runtime-model-only-product-preview-runtime-smoke-corrected-retry-approval-packet\.md/i,
    /runtime-model-only-product-preview-runtime-smoke-remediation\.md/i,
    /This packet is the separate reviewed approval/i,
    /The assessment itself remains non-authorizing/i,
    /No execution may occur in this approval PR/i,
    /runtime_smoke_usefulness_assessment_status: pass/i,
    /runtime_smoke_usefulness_classification: useful/i,
    /runtime_smoke_useful_lenses: signals, maps, plays/i,
    /runtime_smoke_assessment_provider_calls_executed: 0/i,
    /runtime_smoke_assessment_authorizes_provider_call: false/i,
    /runtime_smoke_assessment_authorizes_product_preview_expansion: false/i,
...
test("runtime-smoke tiny expansion approval pins exact future scope, route, dry-run planner, and limits", () => {
  const doc = read(APPROVAL);

  for (const required of [
    /approval_id: runtime-model-only-product-preview-runtime-smoke-tiny-expansion-20260604f/i,
    /approval_kind: three_slot_runtime_model_mode_product_preview_tiny_expansion/i,
    /max_attempts: 1/i,
    ...
    /runtime_mode: product-preview-expansion/i,
    /npm run product-preview:plan/i,
    /product-preview-runtime-smoke-tiny-expansion-20260604f/i,
    /--max-provider-calls 3/i,
    /--max-cost-usd 3/i,
    /--slot-roles representative,edge-case,calibration/i,
    /dry_run: true/i,
...
test("runtime-smoke tiny expansion approval requires screening, stop rules, and no-tool runtime boundary", () => {
  const doc = read(APPROVAL);

  for (const required of [
    /private_source_screening_required_before_each_call: true/i,
    /stop_instead_of_substitute_if_slot_fails_screening: true/i,
    /required_representative_slots: 1/i,
    /required_edge_case_slots: 1/i,
    ...
    /If fewer than all three required roles pass screening/i,
    /tools: false/i,
    /web_search: false/i,
...
test("runtime-smoke tiny expansion approval authorizes only a future bounded slice and preserves non-claims", () => {
  const doc = read(APPROVAL);
  ...
  for (const required of [
    ...
    /future_authorized_tiny_product_preview_expansion: true/i,
    ...
    /graph ingestion readiness/i,
    /multi-account readiness/i,
...
test("usefulness assessment links the later approval while preserving its historical false authorization markers", () => {
  const assessment = read(ASSESSMENT);

  assert.match(assessment, /runtime-model-only-product-preview-runtime-smoke-tiny-expansion-approval-packet\.md/i);
  assert.match(assessment, /later separate docs-only approval/i);
  assert.match(assessment, /authorization markers below remain historical markers/i);
  ...
  assertNoPrivateLeakage("assessment doc", assessment);
});
