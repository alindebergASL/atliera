...
/* Accepted composition: warm neutral surfaces, graphite actions, system typography. */
:root {
  --atl-canvas: #faf9f7;
  --atl-surface: #fdfcfb;
  --atl-ink: #10111c;
  --atl-muted: #5d5c69;
  --atl-line: #e1dfdd;
  --atl-control-line: #85817c;
  --atl-neutral-soft: #eeedeb;
  ...
  --atl-accent: #272731;
  --atl-accent-hover: #3b3b43;
  --atl-accent-soft: #ecebe9;
  ...
  --atl-warning: #512207;
  --atl-warning-soft: #ffb981;
  --atl-error: #b42318;
  --atl-error-soft: #fff1ef;
  --atl-font: Arial, Helvetica, sans-serif;
  --atl-title: 2.375rem;
  --atl-section: 1.25rem;
  --atl-body: 1rem;
  --atl-meta: .875rem;
  ...
  --atl-control-radius: 5px;
  --atl-surface-radius: 5px;
  --atl-focus-ring: 0 0 0 3px #272731;
...
/* Shared-account composition. Keep native service controls and evidence dialog contracts. */
html{scroll-padding-top:88px}
h1,h2,h3{letter-spacing:-.025em}
.workspace-frame{margin:0;background:var(--atl-canvas);padding-bottom:0}
.workspace-header{position:relative;z-index:2;display:flex;flex-wrap:wrap;min-height:64px;padding:8px 40px;gap:12px 24px;background:var(--atl-canvas)}
.workspace-header .brand{order:0;margin:0;font-size:27px;letter-spacing:-.6px}
.workspace-header .account-identity{order:1;font-size:15px;font-weight:400;flex:0 1 auto}
.workspace-header .account-switcher{order:2;font-size:14px}
.workspace-header .workspace-nav{order:3;position:static;display:flex;flex-direction:row;gap:4px;padding:0;border:0;min-height:44px;margin-left:auto;background:transparent}
.workspace-nav a{flex-direction:row;min-height:44px;padding:8px 20px;font-size:15px;border-radius:8px}
.workspace-nav a[aria-current]{box-shadow:none;color:var(--atl-ink);background:var(--atl-selection-surface)}
.workspace-nav svg{display:none}
.workspace-header>a:not(.brand){order:4;font-size:14px}
.account-switcher[open]{padding:0;border:0}
.account-switcher[open] nav,.account-switcher[open]>.meta{background:var(--atl-surface);max-width:100%}
.account-switcher[open] nav{border:1px solid var(--atl-control-line);border-radius:5px}
.account-switcher[open]>.meta{padding:8px;margin:0}
main,.account-workspace,.research-workspace,.workshop-workspace{max-width:1200px;padding:28px 40px 48px}
.storage-notice,.recorded-mode{max-width:1120px;margin:12px auto;padding-inline:0}
.account-readout{padding:0 0 22px;margin:0;border-bottom:1px solid var(--atl-line)}
.account-readout.has-illustration{display:grid;grid-template-columns:minmax(0,1fr) 34%;gap:30px}
.account-readout h1{font-size:40px;line-height:1.16;margin:12px 0 8px}
.account-subtitle{font-size:18px;margin:8px 0 10px}
.account-readout .lede{max-width:60ch;font-size:16px;line-height:1.5}
.account-illustration{margin:0;align-self:center;min-width:0}
.account-illustration img{display:block;width:100%;height:auto;aspect-ratio:395/196;object-fit:cover;border-radius:5px}
.account-illustration figcaption{font-size:12px;color:var(--atl-muted);margin-top:6px}
.research-basis{font-size:14px;gap:8px}.research-basis svg{width:20px;height:20px;fill:none;stroke:currentColor;stroke-width:1.6}
.account-section{margin:20px 0}.account-section>.section-heading h2{font-size:20px}
.priority-grid{display:block}.priority-tile{display:grid;grid-template-columns:38px minmax(0,1fr);gap:8px 24px;padding:22px 0;border:0;border-bottom:1px solid var(--atl-line);border-radius:0}
.priority-copy{display:block;grid-column:2}.priority-copy h3{font-size:20px;margin:0 0 6px}.priority-copy>p{max-width:75ch;font-size:16px;line-height:1.5;margin:0 0 8px}
.priority-tile>.view-details{grid-column:2;min-height:44px;display:flex;align-items:center;margin:0;font-size:15px}
.priority-tile .item-icon,.summary-row .item-icon,.account-confirmation .item-icon{width:32px;height:32px;background:transparent;padding:0;border-radius:0;color:var(--atl-icon-ink)}
.priority-tile .reading-limit{font-size:14px;color:var(--atl-muted);margin-top:8px}
.priority-tile[data-detail-target="detail-redtail-access"] .reading-limit{display:inline-block;max-width:100%;padding:7px 15px;border-radius:24px;background:var(--atl-warning-soft);color:var(--atl-warning)}
.account-columns{gap:32px;margin:20px 0;border-bottom:1px solid var(--atl-line)}
.summary-section{padding:0;border:0;border-radius:0}.summary-section h2{font-size:20px}.summary-row{grid-template-columns:32px minmax(0,1fr);padding:16px 0;border:0}.summary-row h3{font-size:17px;font-weight:600}
.account-confirmation{padding:16px 0;border-radius:0;background:transparent;border-bottom:1px solid var(--atl-line)}
.overview-work [data-work-list-status]:empty{display:none}.overview-work .workshop-item{padding:16px 0}.overview-work>.button{margin-top:12px}
.research-workspace>.lede{max-width:80ch}.research-workspace>section,.research-workspace>.account-reading{max-width:none}
.research-workspace .account-reading{max-width:850px;padding:24px 0}.research-workspace .account-reading h3{font-size:23px}.account-reading>p:not(.meta):not(.support):not(.reading-limit){font-size:17px;line-height:1.65}
.research-workspace .account-reading[data-selected=true]{padding-left:16px}
.research-topics{border:0;padding:0;border-bottom:1px solid var(--atl-line);border-radius:0}
.research-source,.research-inspection-entry{max-width:850px}
a.source-chip,.support a.source-chip{background:transparent;border-radius:4px;text-decoration:underline;text-underline-offset:4px;padding:8px 4px}
.brief-workspace{max-width:860px;padding:28px 40px 48px}.draft-head h1{font-size:30px}.draft-head{padding-bottom:16px;border-bottom:1px solid var(--atl-line)}
.draft-section{border:0;padding:24px 0}.draft-section h2,.checks h2{font-size:16px;color:var(--atl-muted);font-weight:600}
.draft-section p,.questions strong{font-size:18px;line-height:1.75}.questions li{border:0}.questions li:before{background:transparent;color:var(--atl-muted)}
.brief-workspace .review{padding-top:12px}.review textarea{line-height:1.65}
.revision-original,.revision-proposed{border-radius:0;background:var(--atl-surface);border:1px solid var(--atl-line);border-top:3px solid var(--atl-control-line);line-height:1.65}
.revision-proposed{border-top-color:var(--atl-ink)}
dialog[data-evidence-dialog]{background:var(--atl-canvas);box-shadow:none}.evidence-panel-head{background:var(--atl-canvas)}
[data-evidence-content] blockquote{background:var(--atl-neutral-soft);border:0;border-radius:5px;padding:18px;line-height:1.65}
@media(min-width:1440px){body:has(dialog[data-docked][open]) .workspace-frame{margin-right:var(--atl-inspector)}body:has(dialog[data-docked][open]) .workspace-header{gap:8px 16px;padding-inline:24px}body:has(dialog[data-docked][open]) .workspace-header .workspace-nav{margin-left:0}body:has(dialog[data-docked][open]) .account-readout.has-illustration{grid-template-columns:minmax(0,1fr) 32%}body:has(dialog[data-docked][open]) .account-readout h1{font-size:32px}}
@media(max-width:1100px){.workspace-header{padding-inline:24px;gap:8px 16px}.workspace-header .workspace-nav{margin-left:0}.storage-notice,.recorded-mode{margin-inline:24px}.account-readout h1{font-size:34px}}
@media(max-width:700px){
