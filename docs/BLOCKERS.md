...
M5b acceptance remains customer-facing: one real account, public sources fetched through M4, validation and human ratification, durable state, and a shareable Workshop account page with every claim traceable to stored sources and every unverified item visibly labeled. Repository-native prepare/apply is the current mechanism for part of that flow, not a replacement milestone outcome.

The next M5b effects are blocked because `CURRENT_EFFECTIVE_AUTHORIZATION=NONE`. A future real prepare requires an explicit source path and exact size/hash, exact reviewed commit/tree, owner authorization, and one-content-read-per-invocation/no-network/no-provider/no-retry scope. A later apply requires an external human-ratification file named by its exact raw-byte SHA-256 and bound to that exact source pack, candidate, review packet, owner authorization, commit, and tree, plus a separate one-shot local write/read-back/render execution decision. Cross-invocation one-shot source-read authority belongs to the owner decision, not a host replay ledger. Neither decision authorizes acquisition, AWS, providers/models, deployment, control-plane use, or any further graph write.

Current M5b product counters: `M5B_STATUS=IN_PROGRESS`; `REAL_SOURCE_READS=0`; `REAL_GRAPH_WRITES=0`; `REAL_RATIFICATIONS=0`; product provider calls 0; acquisitions 0; deployments/AWS actions 0; retries 0; external/product effects 0. Committed synthetic tests may write only disposable local outputs.

The former host Gate B/v2-r3 package is frozen historical provenance only. No v2-r4 is authorized. Host/archive qualification and V4 reconciliation are closed; the shared control plane remains quarantined and nonblocking. Historical outputs are unratified drafts and cannot satisfy the repository-native ratification boundary.
