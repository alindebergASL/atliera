...
Expiry semantics are deliberately split across slices. Step 3 proves only that `drafted_at <= armed_at < packet_expires_at`; structural verification has no execution timestamp and therefore is **not an execution-time expiry check**. A structurally valid arming can still be presented after its packet has expired. Step 4 must compare its actual execution timestamp against `packet_expires_at` and refuse execution when `execution_at >= packet_expires_at`. This PR remains arming-only and implements no Step 4 execution behavior.

One-shot semantics are likewise split across slices. Step 3 creates and verifies a stable key but has no persistence, registry, database read, or write. Its verifier is pure and idempotent: repeated structural verification records no consumption and cannot enforce replay exclusion. **Only Step 4 can enforce consumption.** Step 4 must check the stable key and atomically record its consumption in the same durable transaction as the authorized effect; it must refuse replay after consumption. The closed boundary markers above make those future execution requirements part of the arming shape without pretending Step 3 has already performed them.
...
- **M5a step 4 — flow execution.** The next slice consumes the armed artifact, the step 2 packet, and the step 1 contract. It is the first M5a slice allowed to run the recorded proposal through the M3 durable-write/reader/render surfaces and the first one that may flip durable-write/render effect markers. Step 4 must re-snapshot and re-verify the arming instead of trusting builder output, compare its actual `execution_at` against `packet_expires_at` and refuse at or after expiry, atomically consume `one_shot_consumption_key` in the same durable transaction as the effect, refuse replay after consumption, and stamp L0 only on completed effects. No Step 4 execution is implemented here.
